...
# Usage:
# "/applications/R/R-3.4.0/bin/Rscript features_EnrichedHeatmap_sorted_by_MNase_Rep1.R MNase_Rep1 2000 genes 3500 2000 2kb '2 kb' 20 20bp promoters"
...
libName <- args[1]
featureNumber <- as.numeric(args[2])
featureName <- args[3]
...
if( region == "promoters" ) {
  ...
  print("The region name provided does not match 'promoter',  'terminator', or 'body'")
}
mat1RegionRowMeans <- rowMeans(mat1Region)
...
# Load feature matrices for each sRNA size class and sort
# by decreasing mat1RegionRowMeans
...
sRNAmatsSorted <- mclapply(seq_along(sRNAmats), function(x) {
  sRNAmats[[x]][sort.int(mat1RegionRowMeans,
                         decreasing = F,
                         index.return = T,
                         na.last = T)$ix,]
}, mc.cores = length(sRNAmats))

for(x in seq_along(sRNAmatsSorted)) {
  attr(sRNAmatsSorted[[x]], "upstream_index") = 1:(upstream/binSize)
  attr(sRNAmatsSorted[[x]], "target_index") = ((upstream/binSize)+1):((upstream+bodyLength)/binSize)
  attr(sRNAmatsSorted[[x]], "downstream_index") = (((upstream+bodyLength)/binSize)+1):(((upstream+bodyLength)/binSize)+(downstream/binSize))
  attr(sRNAmatsSorted[[x]], "extend") = c(upstream, downstream)
  attr(sRNAmatsSorted[[x]], "smooth") = FALSE
  attr(sRNAmatsSorted[[x]], "signal_name") = paste0(sRNAsizes[x], "-nt sRNAs")
  attr(sRNAmatsSorted[[x]], "target_name") = featureName
  attr(sRNAmatsSorted[[x]], "target_is_single_point") = FALSE
  attr(sRNAmatsSorted[[x]], "background") = 0
  attr(sRNAmatsSorted[[x]], "signal_is_categorical") = FALSE
  class(sRNAmatsSorted[[x]]) = c("normalizedMatrix", "matrix")
}
...
# CENH3
  ChIP_col_fun <- colorRamp2(quantile(c(log2ChIPmatsSorted[[3]]),
                                      c(0.1, 0.3, 0.5, 0.7, 0.9, 0.95),
                                      na.rm = T),
                             rich8to6equal)
  pdf(paste0(plotDir, ChIPNames[3], "_around_top", featureNumber, featureName,
             "_heatmap_ordered_by_",
             libName, "_in_", region, ".pdf"),
      width = 5, height = 10)
  geneHeatmap(matSorted = log2ChIPmatsSorted[[3]][1:featureNumber,],
              col_fun = ChIP_col_fun,
              datName = ChIPNamesPlot[3],
              rowOrder = c(1:dim(log2ChIPmatsSorted[[3]][1:featureNumber,])[1])
             )
  dev.off()
