...
mCG_GR <- makeGR(bedgraph = mCG)
mCHG_GR <- makeGR(bedgraph = mCHG)
mCHH_GR <- makeGR(bedgraph = mCHH)
...
fOverlaps_mCG <- fOverlaps(interGR = interGR, datGR = mCG_GR)

profileDNAmeth <- NULL
for(i in seqlevels(interGR)) {
  print(i)
  chr_interGR <- interGR[seqnames(interGR) == chrs[i]]
  # mCG
  ...
  mCG_fOverlaps <- findOverlaps(query = chr_interGR,
                                subject = chr_mCG_GR,
                                type = "any",
                                select = "all",
                                ignore.strand = TRUE)
...
library(segmentSeq)
fOverlaps_mCG_list <- segmentSeq::getOverlaps(coordinates = interGR,
                                              segments = mCG_GR,
                                              overlapType = "overlapping",
                                              whichOverlaps = T,
                                              ignoreStrand = T)

#fOverlaps_mCG_list <- mclapply(seq_along(unique(queryHits(fOverlaps_mCG))),
fOverlaps_mCG_list <- lapply(seq_along(unique(queryHits(fOverlaps_mCG))),
                        function(x) {
                          print(x)
                          subjectHits(fOverlaps_mCG[queryHits(fOverlaps_mCG) == x])
                        })
#                        }, mc.cores = detectCores(), mc.preschedule = T)
