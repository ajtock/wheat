...
# Usage:
# ./TEfams_vs_DMC1_heterochromatin_peaks_bargraphs.R "Heterochromatin DMC1 peaks" "A genome" "B genome" "D genome" Agenome Bgenome Dgenome Agenome heterochromatin 10000

library(ggplot2)
library(ggthemes)

#plotTitle <- "Heterochromatin DMC1 peaks"
#pt1Name <- "A genome"
#pt2Name <- "B genome"
#pt3Name <- "D genome"
#pt1LibName <- "Agenome" 
...
load(paste0(ptOrderDir, "permTest_", perms, "perms_DMC1_Rep1_ChIP_peaks_vs_TEfams_in_",
            ptOrderLibName, "_heterochromatin.RData"))
ptOrder <- ptPeaksOtherPerChrom
ptPeaksOtherPerChrom <- NULL

# Load permutation test results to be used for plotting
load(paste0(pt1Dir, "permTest_", perms, "perms_DMC1_Rep1_ChIP_peaks_vs_TEfams_in_",
            pt1LibName, "_", region, ".RData"))
pt1 <- ptPeaksOtherPerChrom
ptPeaksOtherPerChrom <- NULL

load(paste0(pt2Dir, "permTest_", perms, "perms_DMC1_Rep1_ChIP_peaks_vs_TEfams_in_",
            pt2LibName, "_", region, ".RData"))
pt2 <- ptPeaksOtherPerChrom
ptPeaksOtherPerChrom <- NULL

load(paste0(pt3Dir, "permTest_", perms, "perms_DMC1_Rep1_ChIP_peaks_vs_TEfams_in_",
...
bp <- ggplot(data = df,
  ...
  ggtitle(paste0(plotTitle, " (", prettyNum(as.character(perms),
                                            big.mark = ",", trim = "T"),
                 " permutations)"))
ggsave(paste0(plotDir, "barplot_TEfam_features_permTestResults_",
       ...
       height = 8, width = 14)
save(bp,
     file = paste0(plotDir, "barplot_TEfam_features_permTestResults_",
                   as.character(perms), "perms_",
                   "log2_Observed_Expected_DMC1_Rep1_ChIP_",
                   pt1LibName, "_", pt2LibName, "_", pt3LibName, "_",
                   region, "_peaks.RData"))
