...
# Usage:
# ./others_vs_DMC1_euchromatin_peaks_bargraphs.R "Euchromatin DMC1 peaks" "A genome" "B genome" "D genome" Agenome Bgenome Dgenome Agenome euchromatin 10000

library(ggplot2)
library(ggthemes)

#plotTitle <- "Euchromatin DMC1 peaks"
#pt1Name <- "A genome"
#pt2Name <- "B genome"
#pt3Name <- "D genome"
#pt1LibName <- "Agenome" 
...
pt1Dir <- paste0("/home/ajt200/analysis/wheat/DMC1/snakemake_ChIPseq/mapped/both/peaks/PeakRanger1.18/ranger/p0.001_q0.01/",
                 region, "/regioneR/noMinWidth_mergedOverlaps/")
pt2Dir <- paste0("/home/ajt200/analysis/wheat/DMC1/snakemake_ChIPseq/mapped/both/peaks/PeakRanger1.18/ranger/p0.001_q0.01/",
                 region, "/regioneR/noMinWidth_mergedOverlaps/")
pt3Dir <- paste0("/home/ajt200/analysis/wheat/DMC1/snakemake_ChIPseq/mapped/both/peaks/PeakRanger1.18/ranger/p0.001_q0.01/",
                 region, "/regioneR/noMinWidth_mergedOverlaps/")
...
load(paste0(pt1Dir, "permTest_", perms, "perms_DMC1_Rep1_ChIP_peaks_vs_others_in_",
            pt1LibName, "_", region, ".RData"))
pt1 <- ptPeaksOtherPerChrom[-c(13:16, 18:21, 23:26)]
ptPeaksOtherPerChrom <- NULL

load(paste0(pt2Dir, "permTest_", perms, "perms_DMC1_Rep1_ChIP_peaks_vs_others_in_",
            pt2LibName, "_", region, ".RData"))
pt2 <- ptPeaksOtherPerChrom[-c(13:16, 18:21, 23:26)]
ptPeaksOtherPerChrom <- NULL

load(paste0(pt3Dir, "permTest_", perms, "perms_DMC1_Rep1_ChIP_peaks_vs_others_in_",
            pt3LibName, "_", region, ".RData"))
pt3 <- ptPeaksOtherPerChrom[-c(13:16, 18:21, 23:26)]
ptPeaksOtherPerChrom <- NULL
...
# Order permutation test statistics and feature names by
# decreasing log2(observed/expected) overlaps of wt treatment peaks
pt1_log2ObsExp_sorted <- unlist(pt1_log2ObsExp[sort.int(unlist(ptOrder_log2ObsExp_log2alpha0.05),
                                                        decreasing = T,
                                                        index.return = T)$ix])
pt1_log2alpha0.05_sorted <- unlist(pt1_log2alpha0.05[sort.int(unlist(ptOrder_log2ObsExp_log2alpha0.05),
                                                              decreasing = T,
                                                              index.return = T)$ix])
#pt1_otherNames_sorted <- otherNames[sort.int(unlist(ptOrder_log2ObsExp_log2alpha0.05),
#                                         decreasing = T,
#                                         index.return = T)$ix]
pt1_otherNamesPlot_sorted <- otherNamesPlot[sort.int(unlist(ptOrder_log2ObsExp_log2alpha0.05),
                                                     decreasing = T,
                                                     index.return = T)$ix]
...
# Order permutation test statistics and feature names by
# decreasing log2(observed/expected) overlaps of wt treatment peaks
pt2_log2ObsExp_sorted <- unlist(pt2_log2ObsExp[sort.int(unlist(ptOrder_log2ObsExp_log2alpha0.05),
                                                        decreasing = T,
                                                        index.return = T)$ix])
pt2_log2alpha0.05_sorted <- unlist(pt2_log2alpha0.05[sort.int(unlist(ptOrder_log2ObsExp_log2alpha0.05),
                                                              decreasing = T,
                                                              index.return = T)$ix])
#pt2_otherNames_sorted <- otherNames[sort.int(unlist(ptOrder_log2ObsExp_log2alpha0.05),
#                                         decreasing = T,
#                                         index.return = T)$ix]
pt2_otherNamesPlot_sorted <- otherNamesPlot[sort.int(unlist(ptOrder_log2ObsExp_log2alpha0.05),
                                                 decreasing = T,
                                                 index.return = T)$ix]
...
# Order permutation test statistics and feature names by
# decreasing log2(observed/expected) overlaps of wt treatment peaks
pt3_log2ObsExp_sorted <- unlist(pt3_log2ObsExp[sort.int(unlist(ptOrder_log2ObsExp_log2alpha0.05),
                                                        decreasing = T,
                                                        index.return = T)$ix])
pt3_log2alpha0.05_sorted <- unlist(pt3_log2alpha0.05[sort.int(unlist(ptOrder_log2ObsExp_log2alpha0.05),
                                                              decreasing = T,
                                                              index.return = T)$ix])
#pt3_otherNames_sorted <- otherNames[sort.int(unlist(ptOrder_log2ObsExp_log2alpha0.05),
#                                         decreasing = T,
#                                         index.return = T)$ix]
pt3_otherNamesPlot_sorted <- otherNamesPlot[sort.int(unlist(ptOrder_log2ObsExp_log2alpha0.05),
                                                 decreasing = T,
                                                 index.return = T)$ix]
