...
# Usage:
# /applications/R/R-3.5.0/bin/Rscript cluster1_genes_avgProfileRibbon.R genes_in_Agenome_genomewide 'DMC1_Cluster1_genes' 3500 2000 2kb '2 kb' 20 20bp promoters '1' '4' '0006952' both

#featureName <- "genes_in_Agenome_genomewide"
#featureNamePlot <- "DMC1_Cluster1_genes"
#bodyLength <- 3500
#upstream <- 2000
#downstream <- 2000
#flankName <- "2kb"
...
outDir <- paste0("clusters_by_log2_DMC1_Rep1_ChIP_control_in_",
                 region, "/")
plotDir <- paste0(outDir, "plots/")
system(paste0("[ -d ", outDir, " ] || mkdir ", outDir))
system(paste0("[ -d ", plotDir, " ] || mkdir ", plotDir))

IDs <- as.character(read.table(paste0("clusters_by_log2_DMC1_Rep1_ChIP_control_in_", region,
                                      "/cluster", clusterNo, "_of_", clusterLast,
                                      "_by_log2_DMC1_Rep1_ChIP_control_in_",
                                      region,"_of_", featureName, ".txt"),
                               header = F)$V1)
nonIDs <- as.character(read.table(paste0("clusters_by_log2_DMC1_Rep1_ChIP_control_in_", region,
...
ggObj2_combined_log2ChIP <- lapply(seq_along(ChIPNames), function(x) {
  ...
  theme(
        ...
        plot.margin = unit(c(0.3,1.2,0.0,0.3), "cm"),
        plot.title = element_text(hjust = 0.5, size = 30)) +
  ggtitle(bquote("DMC1 Cluster" * .(clusterLast) ~ "genes (" * italic("n") ~ "=" ~
...
ggsave(paste0(plotDir,
              ...
              "_by_log2_DMC1_Rep1_ChIP_control_in_", region,
