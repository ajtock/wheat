...
# Usage:
# ./cMMb_per_scaled_win_TelCen_arabidopsis.R 10kb 10000 100 100ths red 437
...
COs <- read.table(paste0(inDir, "CO_frequency_genome_",
                         winName, ".txt"))

chrCOprofiles <- mclapply(seq_along(chrs), function(x) {
  chrCOs <- COs[COs$chr == chrs[x],]
}, mc.cores = length(chrs))
chrProfiles <- mclapply(seq_along(chrCOprofiles), function(x) {
  data.frame(chr    = as.character(chrCOprofiles[[x]]$chr),
             window = seq(from = 1,
                          to = chrLens[x],
                          by = winSize),
             cMMb   = as.numeric((chrCOprofiles[[x]]$COs/(numF2s))*100))
}, mc.cores = length(chrCOprofiles))
...
write.table(TelCenMatrix,
            file = paste0("./cMMb_wt_ColLer_", numF2s, "F2_COs_",
                          winName, "_",
                          propName, "_TelCenMatrix.txt"))
# Load TelCenMatrix
TelCenDF <- read.table(paste0("./cMMb_wt_ColLer_", numF2s, "F2_COs_",
                              winName, "_",
                              propName, "_TelCenMatrix.txt"))
...
pdf(paste0("./cMMb_wt_ColLer_", numF2s, "F2_COs_",
           winName, "_",
