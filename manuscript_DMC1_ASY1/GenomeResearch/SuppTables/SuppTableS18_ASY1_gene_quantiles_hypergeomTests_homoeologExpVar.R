...
inDir <- "/home/ajt200/analysis/wheat/ASY1_CS/snakemake_ChIPseq/mapped/geneProfiles_subgenomes/quantiles/quantiles_by_ASY1_in_genes/"
...
summaryTableDFList <- mclapply(seq_along(quantiles), function(x) {
  ...
  summaryTableDFList_geneCat2 <- lapply(seq_along(geneCat2), function(w) {
    load(paste0(inDir2,
                ...
                "_of_4_by_log2_ASY1_CS_Rep1_ChIP_control_in_genes_of_genes_in_Agenome_Bgenome_Dgenome_genomewide_hypergeomTestRes_minConditions6.RData"))
...
colnames(summaryTableDF) <- c("Gene category", "ASY1 gene quantile",
...
write.table(summaryTableDF,
            file = "Supplemental_TableS18_ASY1_gene_quantiles_hypergeometricTests_homoeologExpressionVariation.tsv",
            col.names = T, row.names = F, sep = "\t", quote = F)
write.csv(summaryTableDF,
          file = "Supplemental_TableS18_ASY1_gene_quantiles_hypergeometricTests_homoeologExpressionVariation.csv",
          row.names = F, quote = F)
