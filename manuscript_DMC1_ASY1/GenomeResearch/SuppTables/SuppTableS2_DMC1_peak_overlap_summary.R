...
# Compile DMC1 peak overlap permutation test results into one supplemental table
...
summaryTableDFList <- lapply(seq_along(subgenomes), function(x) {
  R1R3 <- data.frame(Subgenome_compartments = paste0(subgenomes[x], "-genome R1 & R3"),
                     rbind(read.table(paste0("/home/ajt200/analysis/wheat/DMC1/snakemake_ChIPseq/mapped/both/peaks/",
                                             "PeakRanger1.18/ranger/p0.001_q0.01/euchromatin/regioneR/noMinWidth_mergedOverlaps/",
                                             "permTest_10000perms_DMC1_Rep1_ChIP_peaks_vs_others_in_",
                                             subgenomes[x],"genome_euchromatin_DataFrame.txt"), header = T, stringsAsFactors = F),
                           read.table(paste0("/home/ajt200/analysis/wheat/DMC1/snakemake_ChIPseq/mapped/both/peaks/",
                                             "PeakRanger1.18/ranger/p0.001_q0.01/euchromatin/regioneR/noMinWidth_mergedOverlaps/",
                                             "permTest_10000perms_DMC1_Rep1_ChIP_peaks_vs_TEfams_in_",
                                             subgenomes[x],"genome_euchromatin_DataFrame.txt"), header = T, stringsAsFactors = F)),
                     stringsAsFactors = F)
...
write.table(summaryTableDF,
            file = "Supplemental_TableS2_DMC1_peak_overlap_summary.tsv",
            col.names = T, row.names = F, sep = "\t", quote = F)
write.csv(summaryTableDF,
          file = "Supplemental_TableS2_DMC1_peak_overlap_summary.csv",
          row.names = F, quote = F)
