...
# Compile ASY1 gene quantile GOslim enrichment analysis results into one supplemental table
...
summaryTableDFList <- lapply(seq_along(quantiles), function(x) {
  Wg <- data.frame(Subgenome = "All",
                   ...
                   read.table(paste0("/home/ajt200/analysis/wheat/ASY1_CS/snakemake_ChIPseq/mapped/geneProfiles_subgenomes/quantiles/",
                                     "quantiles_by_log2_ASY1_CS_Rep1_ChIP_control_in_genes/GOslim/featureIDs_quantile", x, "_of_4_by_log2_ASY1_CS_Rep1_ChIP_control_in_genes_of_genes_in_",
...
colnames(summaryTableDF) <- c("Subgenome", "ASY1 gene quantile",
                              "GO ID", "GO term",
                              "Annotated genes in subgenome", "Expected genes in quantile", "Observed genes in quantile", "P")
print(colnames(summaryTableDF))
write.table(summaryTableDF,
            file = "Supplemental_TableS9_ASY1_gene_quantiles_GOslim.tsv",
            col.names = T, row.names = F, sep = "\t", quote = F)
write.csv(summaryTableDF,
          file = "Supplemental_TableS9_ASY1_gene_quantiles_GOslim.csv",
          row.names = F, quote = F)
