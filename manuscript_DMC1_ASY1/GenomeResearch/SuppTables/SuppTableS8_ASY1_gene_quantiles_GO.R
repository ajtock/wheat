...
# Compile ASY1 gene quantile GO enrichment analysis results into one supplemental table
...
summaryTableDFList <- lapply(seq_along(quantiles), function(x) {
  Wg <- data.frame(Subgenome = "All",
                   ...
                   read.table(paste0("/home/ajt200/analysis/wheat/ASY1_CS/snakemake_ChIPseq/mapped/geneProfiles_subgenomes/quantiles/",
                                     "quantiles_by_log2_ASY1_CS_Rep1_ChIP_control_in_genes/GO/featureIDs_quantile", x, "_of_4_by_log2_ASY1_CS_Rep1_ChIP_control_in_genes_of_genes_in_",
...
write.table(summaryTableDF,
            file = "Supplemental_TableS8_ASY1_gene_quantiles_GO.tsv",
            col.names = T, row.names = F, sep = "\t", quote = F)
write.csv(summaryTableDF,
          file = "Supplemental_TableS8_ASY1_gene_quantiles_GO.csv",
          row.names = F, quote = F)
