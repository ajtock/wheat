...
# Compile cMMb gene quantile GOslim enrichment analysis results into one supplemental table
...
summaryTableDFList <- lapply(seq_along(quantiles), function(x) {
  Wg <- data.frame(Subgenome = "All",
                   ...
                   read.table(paste0("/home/ajt200/analysis/wheat/ASY1_CS/snakemake_ChIPseq/mapped/geneProfiles_subgenomes/quantiles/",
                                     "quantiles_by_cMMb/GOslim/featureIDs_quantile", x, "_of_4_by_cMMb_of_genes_in_",
...
write.table(summaryTableDF,
            file = "Supplemental_TableS5_cMMb_gene_quantiles_GOslim.tsv",
            col.names = T, row.names = F, sep = "\t", quote = F)
write.csv(summaryTableDF,
          file = "Supplemental_TableS5_cMMb_gene_quantiles_GOslim.csv",
          row.names = F, quote = F)
