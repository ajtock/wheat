...
# Compile H3K27me3 gene quantile hypergeometric test results into one supplemental table
...
summaryTableDFList <- mclapply(seq_along(geneCat), function(w) {
  summaryTableDFList_geneCat <- lapply(seq_along(quantiles), function(x) {
    load(paste0("/home/ajt200/analysis/wheat/ASY1_CS/snakemake_ChIPseq/mapped/geneProfiles_subgenomes/quantiles/",
                ...
                "_of_4_by_log2_H3K27me3_ChIP_SRR6350666_control_in_genes_of_genes_in_Agenome_Bgenome_Dgenome_genomewide_hypergeomTestRes.RData"))
    Wg <- hgTestResults
    rm(hgTestResults); gc()
    Wg <- data.frame(
                     gene_category = geneCatName[w],
                     subgenome = "All",
                     H3K27me3_gene_quantile = paste0("Quantile ", x),
    ...
    load(paste0("/home/ajt200/analysis/wheat/ASY1_CS/snakemake_ChIPseq/mapped/geneProfiles_subgenomes/quantiles/",
                "quantiles_by_H3K27me3_in_genes/hypergeometricTests/", geneCat[w], "_representation_among_quantile", x,
                "_of_4_by_log2_H3K27me3_ChIP_SRR6350666_control_in_genes_of_genes_in_Agenome_genomewide_hypergeomTestRes.RData"))
    Ag <- hgTestResults
    rm(hgTestResults); gc()
    Ag <- data.frame(
                     gene_category = geneCatName[w],
    ...
    load(paste0("/home/ajt200/analysis/wheat/ASY1_CS/snakemake_ChIPseq/mapped/geneProfiles_subgenomes/quantiles/",
                "quantiles_by_H3K27me3_in_genes/hypergeometricTests/", geneCat[w], "_representation_among_quantile", x,
                "_of_4_by_log2_H3K27me3_ChIP_SRR6350666_control_in_genes_of_genes_in_Bgenome_genomewide_hypergeomTestRes.RData"))
    Bg <- hgTestResults
    rm(hgTestResults); gc()
    Bg <- data.frame(
                     gene_category = geneCatName[w],
    ...
    load(paste0("/home/ajt200/analysis/wheat/ASY1_CS/snakemake_ChIPseq/mapped/geneProfiles_subgenomes/quantiles/",
                "quantiles_by_H3K27me3_in_genes/hypergeometricTests/", geneCat[w], "_representation_among_quantile", x,
                "_of_4_by_log2_H3K27me3_ChIP_SRR6350666_control_in_genes_of_genes_in_Dgenome_genomewide_hypergeomTestRes.RData"))
    Dg <- hgTestResults
    rm(hgTestResults); gc()
    Dg <- data.frame(
                     gene_category = geneCatName[w],
...
colnames(summaryTableDF) <- c("Gene category", "Subgenome", "H3K27me3 gene quantile",
...
write.table(summaryTableDF,
            file = "Supplemental_TableS15_H3K27me3_gene_quantiles_hypergeometricTests.tsv",
            col.names = T, row.names = F, sep = "\t", quote = F)
write.csv(summaryTableDF,
          file = "Supplemental_TableS15_H3K27me3_gene_quantiles_hypergeometricTests.csv",
          row.names = F, quote = F)
