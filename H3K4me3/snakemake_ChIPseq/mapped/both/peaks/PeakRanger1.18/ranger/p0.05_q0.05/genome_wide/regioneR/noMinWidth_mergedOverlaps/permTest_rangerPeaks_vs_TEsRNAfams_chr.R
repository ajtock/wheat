...
# csmit -m 100G -c 48 "/applications/R/R-3.3.2/bin/Rscript permTest_rangerPeaks_vs_TEsRNAfams_chr.R H3K4me3_Rep1_ChIP chr1A"
...
system(paste0("[ -d ", chrName, "/plots/TEsRNA ] || mkdir ", chrName, "/plots/TEsRNA")
)
outDir <- paste0("./", chrName, "/")
RNAplotDir <- paste0(outDir, "plots/TEsRNA/")
...
TEsRNAGR <- lapply(seq_along(RNAfamNames), function(x) {
  TEsRNA <- read.table(paste0(TEdir,
                              "iwgsc_refseqv1.0_TransposableElements_2017Mar13_superfamily_",
                              RNAfamNames[x], ".bed"), header = F)
  TEsRNAGRtmp <- GRanges(seqnames = TEsRNA$V1,
                         ranges = IRanges(start = TEsRNA$V2+1,
                                          end = TEsRNA$V3),
                         strand = "*")
  TEsRNAGRtmp[seqnames(TEsRNAGRtmp) == chrName]
})
...
for(i in 1:length(ptPeaksTEsRNAPerChrom)) {
  pdf(paste0(RNAplotDir, RNAfamNames[i],
             "_permTest_nperm10000_", ChIPLibName,
             "_rangerPeaks_perChrom_", chrName, ".pdf"),
      width = 10, height = 7)
  plot(ptPeaksTEsRNAPerChrom[[i]], main = paste0(ChIPLibName, " rangerPeaks vs ", RNAfamNames[i]), xlab = "Number of overlaps", ylab = "Density")
  dev.off()
  ...
  plot(lz_1kb, main = paste0(ChIPLibName, " rangerPeaks vs ", RNAfamNames[i], " (1-kb shift)"))
  mtext(side = 3, at = 2, text = paste0(ChIPLibName, " rangerPeaks vs ", RNAfamNames[i], " (1-kb shift)"))
  plot(lz_10kb, main = paste0(ChIPLibName, " rangerPeaks vs ", RNAfamNames[i], " (10-kb shift)"))
  mtext(side = 3, at = 2, text = paste0(ChIPLibName, " rangerPeaks vs ", RNAfamNames[i], " (10-kb shift)"))
  plot(lz_custom, main = paste0(ChIPLibName, " rangerPeaks vs ", RNAfamNames[i], " (~", win, "-kb shift)"))
  mtext(side = 3, at = 2, text = paste0(ChIPLibName, " rangerPeaks vs ", RNAfamNames[i], " (~", win, "-kb shift)"))
  dev.off()
}
