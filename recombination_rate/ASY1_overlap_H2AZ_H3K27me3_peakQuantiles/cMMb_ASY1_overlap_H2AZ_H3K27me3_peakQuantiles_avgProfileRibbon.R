...
# Usage:
# cMMb_ASY1_overlap_H2AZ_H3K27me3_peakQuantiles_avgProfileRibbon.R 100kb 1 4 ASY1_CS ASY1_CS_Rep1_ChIP input H3_input_SRR6350669 both purple4 ASY1_CS_peaks 'ASY1 peaks' 20bp 20 2kb '2 kb' 2000 'A' 'heterochromatin' '0.02,0.94'
...
genomeName <- args[17]
region <- args[18]
legendPos <- as.numeric(unlist(strsplit(args[19],
                                        split = ",")))
...
## ChIP
if(libNameChIP %in% c("H4K4me3_ChIP_SRR6350668",
                      "H3K27me3_ChIP_SRR6350666",
                      "H3K36me3_ChIP_SRR6350670",
                      "H3K9ac_ChIP_SRR6350667",
                      "CENH3_ChIP_SRR1686799")) {
  covDirChIP <- paste0("/home/ajt200/analysis/wheat/epigenomics_shoot_leaf_IWGSC_2018_Science/",
                       markChIP, "/snakemake_ChIPseq/mapped/ASY1peakProfiles_overlap_H2AZ_H3K27me3_peaks/matrices/")
} else {
...
if(libNameControl == "H3_input_SRR6350669") {
  covDirControl <- paste0("/home/ajt200/analysis/wheat/epigenomics_shoot_leaf_IWGSC_2018_Science/",
                          "input/snakemake_ChIPseq/mapped/ASY1peakProfiles_overlap_H2AZ_H3K27me3_peaks/matrices/")
} else if(libNameControl == "MNase_Rep1") {
  covDirControl <- paste0("/home/ajt200/analysis/wheat/",
                          "MNase/snakemake_ChIPseq/mapped/ASY1peakProfiles_overlap_H2AZ_H3K27me3_peaks/matrices/")
} else {
...
# Add peakIDs
log2covMat <- data.frame(peakID = peaks$name,
                         log2covMat)
# Subdivide coverage matrix into above-defined quantiles
log2covMat_quantiles <- lapply(seq_along(1:quantiles), function(j) {
   tmp1 <- log2covMat[log2covMat$peakID %in% peaksDF_quantiles[[j]]$peakID,]
...
log2covMat_quantiles <- lapply(seq_along(1:quantiles), function(j) {
  log2covMat_quantiles[[j]][order(peaksDF_quantiles[[j]]$cMMb,
                                  decreasing = T,
                                  na.last = NA),]
})
...
#                  as.character(log2covMat_quantiles[[j]]$peakID)))
#})
...
data.frame(window = colnames(as.matrix(log2covMat_quantiles[[j]][,-1])),
           t(as.matrix(log2covMat_quantiles[[j]][,-1])))
})
...
log2covMat_randomPCs <- lapply(seq_along(1:quantiles), function(j) {
  tmp1 <- log2covMat[log2covMat$peakID %in% peaksDF_randomPCs[[j]]$peakID,]
  tmp1[order(tmp1$peakID,
             decreasing = F,
             na.last = NA),]
})
...
data.frame(window = colnames(as.matrix(log2covMat_randomPCs[[j]][,-1])),
           t(as.matrix(log2covMat_randomPCs[[j]][,-1])))
})
...
ggsave(paste0(plotDir,
              ...
              "_around_", genomeName, "genome_", region, "_",
