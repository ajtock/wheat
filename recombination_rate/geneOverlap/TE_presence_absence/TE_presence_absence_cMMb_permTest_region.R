...
# Usage:
#./TE_presence_absence_cMMb_permTest_region.R 100kb 1 Mutator DTM 500000 10000 0.0001 'euchromatin'
...
TEsGR <- GRanges(seqnames = TEs$chr,
                 ...
                 strand = TEs$strand)
# Subset to include only those overlapping specified region (e.g., euchromatin)
TEsGR <- subsetByOverlaps(x = TEsGR,
                          ranges = regionGR,
                          type = "any",
                          invert = FALSE,
                          ignore.strand = TRUE)
...
loci_cMMb_permTest <- function(targets,
                               ...
                               resultsDir,
                               plotDir) {
  #targets = promoter_TE_overlapsGR[grep(genomeNames[z],
  #                                      seqnames(promoter_TE_overlapsGR))]
  #nontargets = promoter_TE_nonoverlapsGR_nearby[grep(genomeNames[z],
  #                                                   seqnames(promoter_TE_nonoverlapsGR_nearby))]
  #targetsName = paste0("gene_promoters_in_", genomeNames[z], "genome_", region,
  #                     "_overlapping_", superfamName, "_", superfamCode, "_TEs")
  #targetsNamePlot = paste0("Gene promoters in ", genomeNamesPlot[z], "-genome ", region,
  #                         " overlapping ", superfamName, " (", superfamCode, ") TEs")
...
sapply(seq_along(genomeNames), function(z) {
  loci_cMMb_permTest(targets = promoter_TE_overlapsGR[grep(genomeNames[z],
                                                           seqnames(promoter_TE_overlapsGR))],
                     nontargets = promoter_TE_nonoverlapsGR_nearby[grep(genomeNames[z],
                                                                        seqnames(promoter_TE_nonoverlapsGR_nearby))],
                     targetsName = paste0("gene_promoters_in_", genomeNames[z], "genome_", region,
                     ...
                     resultsDir = promoterResDir,
                     plotDir = promoterPlotDir)
  loci_cMMb_permTest(targets = terminator_TE_overlapsGR[grep(genomeNames[z],
                                                             seqnames(terminator_TE_overlapsGR))],
                     nontargets = terminator_TE_nonoverlapsGR_nearby[grep(genomeNames[z],
                                                                          seqnames(terminator_TE_nonoverlapsGR_nearby))],
