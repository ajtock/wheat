...
# Usage:
# ./chrProfilePlot_1homoeologuegroup_log2_histoneMod_IWGSCinput_x2_NLRs_stress_response_genes.R H3K4me3 H3K4me3_Rep1_ChIP H3K9me2 H3K9me2_Rep1_ChIP MNase MNase_Rep1 MNase MNase_Rep1 both 1Mb 1000000 15 forestgreen magenta3 180220 Gypsy_LTR_RLG 'chr3A,chr3B,chr3D'
# ./chrProfilePlot_1homoeologuegroup_log2_histoneMod_IWGSCinput_x2_NLRs_stress_response_genes.R H3K27me1 H3K27me1_Rep1_ChIP H3K27me3 H3K27me3_ChIP_SRR6350666 MNase MNase_Rep1 input H3_input_SRR6350669 both 1Mb 1000000 15 firebrick1 navy 180220 Gypsy_LTR_RLG 'chr3A,chr3B,chr3D'
...
feature1 <- read.table(paste0("/home/ajt200/analysis/wheat/chromosomeProfiles/genes/NLR_gene_frequency_per_",
                              winName, ".txt"),
                       header = T)
colnames(feature1) <- c("chr", "window", "features")
feature2 <- read.table(paste0("/home/ajt200/analysis/wheat/chromosomeProfiles/genes/stress_response_gene_frequency_per_",
                              winName, ".txt"),
                       header = T)
colnames(feature2) <- c("chr", "window", "features")

chrProfilesFeature1 <- mclapply(seq_along(chrs), function(x) {
  feature1[feature1$chr == chrs[x],]
}, mc.cores = length(chrs))

filt_chrProfilesFeature1 <- mclapply(seq_along(chrProfilesFeature1), function(x) {
  filt_chrProfileFeature1 <- stats::filter(x = chrProfilesFeature1[[x]]$features,
                                           filter = f,
                                           sides = 2)
  filt_chrProfileFeature1[1:flank] <- filt_chrProfileFeature1[flank+1]
  filt_chrProfileFeature1[(length(filt_chrProfileFeature1)-flank+1):length(filt_chrProfileFeature1)] <- filt_chrProfileFeature1[(length(filt_chrProfileFeature1)-flank)]
  data.frame(chr = as.character(chrProfilesFeature1[[x]]$chr),
             window = as.integer(chrProfilesFeature1[[x]]$window),
             filt_feature = as.numeric(filt_chrProfileFeature1),
             stringsAsFactors = F)
}, mc.cores = length(chrProfilesFeature1))

chrProfilesFeature2 <- mclapply(seq_along(chrs), function(x) {
  feature2[feature2$chr == chrs[x],]
}, mc.cores = length(chrs))

filt_chrProfilesFeature2 <- mclapply(seq_along(chrProfilesFeature2), function(x) {
  filt_chrProfileFeature2 <- stats::filter(x = chrProfilesFeature2[[x]]$features,
                                           filter = f,
                                           sides = 2)
  filt_chrProfileFeature2[1:flank] <- filt_chrProfileFeature2[flank+1]
  filt_chrProfileFeature2[(length(filt_chrProfileFeature2)-flank+1):length(filt_chrProfileFeature2)] <- filt_chrProfileFeature2[(length(filt_chrProfileFeature2)-flank)]
  data.frame(chr = as.character(chrProfilesFeature2[[x]]$chr),
             window = as.integer(chrProfilesFeature2[[x]]$window),
             filt_feature = as.numeric(filt_chrProfileFeature2),
             stringsAsFactors = F)
}, mc.cores = length(chrProfilesFeature2))

minFeature <- min(unlist(mclapply(seq_along(filt_chrProfilesFeature1),
  function(x) {
    min(c(filt_chrProfilesFeature1[[x]]$filt_feature,
          filt_chrProfilesFeature2[[x]]$filt_feature), na.rm = T)
}, mc.cores = length(filt_chrProfilesFeature1))))
maxFeature <- max(unlist(mclapply(seq_along(filt_chrProfilesFeature1),
  function(x) {
    max(c(filt_chrProfilesFeature1[[x]]$filt_feature,
          filt_chrProfilesFeature2[[x]]$filt_feature), na.rm = T)
}, mc.cores = length(filt_chrProfilesFeature1))))
...
for(x in which(chrs %in% chrName)) {
  chrPartitionPlotCov2_feature2(chrx = which(chrs %in% chrName),
                               ...
                               legendLoc = "top",
                               legendLabs = c(markChIPA, markChIPB, "NLR-encoding", "Stress response"),
                               xplot2 = filt_chrProfilesFeature1[[x]]$window,
                               dat2A = filt_chrProfilesFeature1[[x]]$filt_feature,
                               col2A = "steelblue1",
                               dat2B = filt_chrProfilesFeature2[[x]]$filt_feature,
                               col2B = "springgreen",
                               Ylab2 = "Genes",
