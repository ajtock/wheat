...
# Usage:
# ./chrProfilePlot_1homoeologuegroup_log2_ChIP_input_cMMb_genes_step1Mb_XMvar_4profiles.R DMC1 DMC1_Rep1_ChIP input H3_input_SRR6350669 1Mb 1000000 15 120421 'chr3A,chr3B,chr3D'
...
covDirChIPB <- paste0("/home/ajt200/analysis/wheat/",
                      markChIP, "/snakemake_ChIPseq_MAPQ0_XM0/mapped/",
                      "unique/bg/")
...
covDirControlB <- paste0("/home/ajt200/analysis/wheat/epigenomics_shoot_leaf_IWGSC_2018_Science/",
                         markControl, "/snakemake_ChIPseq_MAPQ0_XM0/mapped/unique/bg/")
...
## ChIPD profile
covDirChIPD <- paste0("/home/ajt200/analysis/wheat/",
                      markChIP, "/snakemake_ChIPseq_MAPQ0_XM0/mapped/",
                      "both/bg/")
profileChIPD <- read.table(paste0(covDirChIPD, libNameChIP, "_MappedOn_wheat_v1.0_lowXM_",
                                  "both_sort_norm_binSize", winName, ".bedgraph"))
# Rows where the difference between end and start coordinates is > winSize
profileChIPD_bigWins <- profileChIPD[profileChIPD$V3-profileChIPD$V2 > winSize,]
# Rows where the difference between end and start coordinates is == winSize
profileChIPD <- profileChIPD[profileChIPD$V3-profileChIPD$V2 == winSize,]

# Create a list of big windows, each split into windows of winSize,
# or < winSize if at chromosome end
profileChIPD_bigWinsList <- mclapply(seq_along(1:dim(profileChIPD_bigWins)[1]), function(x) {
  bigWinsSplit <- seq(from = profileChIPD_bigWins[x,]$V2,
                      to = profileChIPD_bigWins[x,]$V3,
                      by = winSize)

  if(bigWinsSplit[length(bigWinsSplit)] < profileChIPD_bigWins[x,]$V3) {
    data.frame(V1 = as.character(profileChIPD_bigWins[x,]$V1),
               V2 = as.integer(c(bigWinsSplit[-length(bigWinsSplit)],
                                 bigWinsSplit[length(bigWinsSplit)])),
               V3 = as.integer(c(bigWinsSplit[-length(bigWinsSplit)]+winSize,
                                 profileChIPD_bigWins[x,]$V3)),
               V4 = as.numeric(profileChIPD_bigWins[x,]$V4))
  } else if (bigWinsSplit[length(bigWinsSplit)] == profileChIPD_bigWins[x,]$V3) {
    data.frame(V1 = as.character(profileChIPD_bigWins[x,]$V1),
               V2 = as.integer(bigWinsSplit[-length(bigWinsSplit)]),
               V3 = as.integer(bigWinsSplit[-length(bigWinsSplit)]+winSize),
               V4 = as.numeric(profileChIPD_bigWins[x,]$V4))
...
profileChIPD_bigWinsDT <- rbindlist(profileChIPD_bigWinsList)
profileChIPD <- rbind.fill(profileChIPD, profileChIPD_bigWinsDT)
profileChIPD <- profileChIPD[order(profileChIPD$V1, profileChIPD$V2),]

chrLenValsAList <- mclapply(seq_along(chrs), function (x) {
  chrProfileChIPD <- profileChIPD[profileChIPD$V1 == chrs[x],]
  if(chrProfileChIPD[dim(chrProfileChIPD)[1],]$V3 < chrLens[x]) {
    data.frame(V1 = chrs[x],
               V2 = as.integer(chrProfileChIPD[dim(chrProfileChIPD)[1],]$V3),
               V3 = as.integer(chrLens[x]),
               V4 = as.numeric(chrProfileChIPD[dim(chrProfileChIPD)[1],]$V4))
  }
}, mc.cores = detectCores())
profileChIPD_chrLenValsADT <- rbindlist(chrLenValsAList)
profileChIPD <- rbind.fill(profileChIPD, profileChIPD_chrLenValsADT)
profileChIPD <- profileChIPD[order(profileChIPD$V1, profileChIPD$V2),]

profileChIPD <- data.frame(chr = as.character(profileChIPD$V1),
                           window = as.integer(profileChIPD$V2+1),
                           CPM = as.numeric(profileChIPD$V4),
                           stringsAsFactors = F)

## ControlD profile
covDirControlD <- paste0("/home/ajt200/analysis/wheat/epigenomics_shoot_leaf_IWGSC_2018_Science/",
                         markControl, "/snakemake_ChIPseq_MAPQ0_XM0/mapped/both/bg/")
profileControlD <- read.table(paste0(covDirControlD, libNameControl, "_MappedOn_wheat_v1.0_lowXM_",
                                     "both_sort_norm_binSize", winName, ".bedgraph"))
# Rows where the difference between end and start coordinates is > winSize
profileControlD_bigWins <- profileControlD[profileControlD$V3-profileControlD$V2 > winSize,]
# Rows where the difference between end and start coordinates is == winSize
profileControlD <- profileControlD[profileControlD$V3-profileControlD$V2 == winSize,]

# Create a list of big windows, each split into windows of winSize,
# or < winSize if at chromosome end
profileControlD_bigWinsList <- mclapply(seq_along(1:dim(profileControlD_bigWins)[1]), function(x) {
  bigWinsSplit <- seq(from = profileControlD_bigWins[x,]$V2,
                      to = profileControlD_bigWins[x,]$V3,
                      by = winSize)

  if(bigWinsSplit[length(bigWinsSplit)] < profileControlD_bigWins[x,]$V3) {
    data.frame(V1 = as.character(profileControlD_bigWins[x,]$V1),
               V2 = as.integer(c(bigWinsSplit[-length(bigWinsSplit)],
                                 bigWinsSplit[length(bigWinsSplit)])),
               V3 = as.integer(c(bigWinsSplit[-length(bigWinsSplit)]+winSize,
                                 profileControlD_bigWins[x,]$V3)),
               V4 = as.numeric(profileControlD_bigWins[x,]$V4))
  } else if (bigWinsSplit[length(bigWinsSplit)] == profileControlD_bigWins[x,]$V3) {
    data.frame(V1 = as.character(profileControlD_bigWins[x,]$V1),
               V2 = as.integer(bigWinsSplit[-length(bigWinsSplit)]),
               V3 = as.integer(bigWinsSplit[-length(bigWinsSplit)]+winSize),
               V4 = as.numeric(profileControlD_bigWins[x,]$V4))
...
profileControlD_bigWinsDT <- rbindlist(profileControlD_bigWinsList)
profileControlD <- rbind.fill(profileControlD, profileControlD_bigWinsDT)
profileControlD <- profileControlD[order(profileControlD$V1, profileControlD$V2),]

chrLenValsAList <- mclapply(seq_along(chrs), function (x) {
  chrProfileControl <- profileControlD[profileControlD$V1 == chrs[x],]
  if(chrProfileControl[dim(chrProfileControl)[1],]$V3 < chrLens[x]) {
    data.frame(V1 = chrs[x],
               V2 = as.integer(chrProfileControl[dim(chrProfileControl)[1],]$V3),
               V3 = as.integer(chrLens[x]),
...
profileControlD_chrLenValsADT <- rbindlist(chrLenValsAList)
profileControlD <- rbind.fill(profileControlD, profileControlD_chrLenValsADT)
profileControlD <- profileControlD[order(profileControlD$V1, profileControlD$V2),]

profileControlD <- data.frame(chr = as.character(profileControlD$V1),
                              window = as.integer(profileControlD$V2+1),
                              CPM = as.numeric(profileControlD$V4),
                              stringsAsFactors = F)

## ChIPD
# Get coverage within each window
profileChIPD <- data.frame(chr = as.character(profileChIPD$chr),
                           window = as.numeric(profileChIPD$window),
                           CPM = as.numeric(log2((profileChIPD$CPM+1)/(profileControlD$CPM+1))),
                           stringsAsFactors = F)
                        
chrProfilesChIPD <- mclapply(seq_along(chrs), function(x) {
  profileChIPD[profileChIPD$chr == chrs[x],]
}, mc.cores = length(chrs))
...
filt_chrProfilesChIPD <- mclapply(seq_along(chrProfilesChIPD), function(x) {
  filt_chrProfile <- stats::filter(x = chrProfilesChIPD[[x]]$CPM,
                                   filter = f,
                                   sides = 2)
  filt_chrProfile[1:flank] <- filt_chrProfile[flank+1]
  filt_chrProfile[(length(filt_chrProfile)-flank+1):length(filt_chrProfile)] <- filt_chrProfile[(length(filt_chrProfile)-flank)]
  data.frame(chr = as.character(chrProfilesChIPD[[x]]$chr),
             window = as.integer(chrProfilesChIPD[[x]]$window),
             filt_CPM = as.numeric(filt_chrProfile),
             stringsAsFactors = F)
}, mc.cores = length(chrProfilesChIPD))
...
minCPM_chrs <- min(unlist(mclapply(seq_along(filt_chrProfilesChIPA),
  function(x) {
    min(c(filt_chrProfilesChIPA[[x]]$filt_CPM,
          ...
          filt_chrProfilesChIPD[[x]]$filt_CPM))
...
#                                 rug1Col = "grey40",
                                 ...
                                 col1A = makeTransparent("darkorange4"),
                                 dat1B = filt_chrProfilesChIPB[[x]]$filt_CPM,
                                 col1B = makeTransparent("darkorange"),
                                 dat1C = filt_chrProfilesChIPC[[x]]$filt_CPM,
                                 col1C = makeTransparent("darkgreen"),
                                 dat1D = filt_chrProfilesChIPD[[x]]$filt_CPM,
                                 col1D = makeTransparent("limegreen"),
                                 Ylab1 = bquote("Log"[2]*"("*.(sub("_\\w+", "", markChIP))*" ChIP/input)"),
                                 min1 = -max((minCPM*-1), maxCPM),
                                 max1 = maxCPM,
                                 legendLoc = "topright",
                                 legendLabs = c(expression("MAPQ">=23 ~ ~ "XM"<=6), expression("MAPQ">=23 ~ ~ "XM = 0"),
                                                expression("MAPQ">=0 ~ ~ "XM"<=6), expression("MAPQ">=0 ~ ~ "XM = 0"),
                                                "cM/Mb", "Genes"),
...
for(x in which(chrs %in% chrs)) {
  chrPartitionPlotCov4l_feature2(chrx = which(chrs %in% chrs),
                                 title = sub("c", "C", chrs[x]),
                                 cenStart = centromereStart[x],
                                 cenEnd = centromereEnd[x],
#                                 rug1 = markers[markers$chromosome == chrs[x],]$physicalPosition,
