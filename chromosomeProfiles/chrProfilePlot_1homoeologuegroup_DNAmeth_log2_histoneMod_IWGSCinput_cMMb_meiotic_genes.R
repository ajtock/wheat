...
mCG <- read.table(gzfile(paste0(DNAmethDir,
                                ...
                                DNAmethContexts[1], ".gz.bismark.cov.gz")),
...
mCHG <- read.table(gzfile(paste0(DNAmethDir,
                                 DNAmethName, "_MappedOn_wheat_v1.0_incl_organelles_controls_dedup_",
                                 DNAmethContexts[2], ".gz.bismark.cov.gz")),
                   header = F, stringsAsFactors = F,
                   colClasses = c(rep(NA, 2), "NULL", NA, rep("NULL", 2)))
...
mCG_profile <- NULL
for(x in 1:length(chrs)) {
  chr_windowsGR <- windowsGR[seqnames(windowsGR) == chrs[x]]
  chr_mCG <- mCG[mCG$chr == chrs[x],]
  chr_mCG_GR <- GRanges(seqnames = chrs[x],
                        ranges = IRanges(start = chr_mCG$pos,
                                         width = 1),
                        strand = "*")
  # Identify overlapping windows and cytosine coordinates
  fOverlaps <- findOverlaps(query = chr_windowsGR,
                            subject = chr_mCG_GR,
                            type = "any",
                            select = "all",
                            ignore.strand = TRUE)
