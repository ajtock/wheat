...
# Usage:
# ./chrProfilePlot_1homoeologuegroup_log2_histoneMod_IWGSCinput_x2_genes_TEs.R H3K4me3 H3K4me3_Rep1_ChIP H3K9me2 H3K9me2_Rep1_ChIP MNase MNase_Rep1 MNase MNase_Rep1 both 1Mb 1000000 15 forestgreen magenta3 180220 Gypsy_LTR_RLG 'chr3A,chr3B,chr3D'
# ./chrProfilePlot_1homoeologuegroup_log2_histoneMod_IWGSCinput_x2_genes_TEs.R H3K27me1 H3K27me1_Rep1_ChIP H3K27me3 H3K27me3_ChIP_SRR6350666 MNase MNase_Rep1 input H3_input_SRR6350669 both 1Mb 1000000 15 firebrick1 navy 180220 Gypsy_LTR_RLG 'chr3A,chr3B,chr3D'
...
feature1 <- read.table(paste0("/home/ajt200/analysis/wheat/chromosomeProfiles/genes/gene_frequency_per_",
                              winName, ".txt"),
                       header = T)
colnames(feature1) <- c("chr", "window", "features")
feature2 <- read.table(paste0("/home/ajt200/analysis/wheat/chromosomeProfiles/TEs/superfamilies/TE_frequency_per_",
...
minFeature <- min(unlist(mclapply(seq_along(filt_chrProfilesFeature1),
  function(x) {
    min(c(filt_chrProfilesFeature1[[x]]$filt_feature), na.rm = T)
}, mc.cores = length(filt_chrProfilesFeature1))))
maxFeature <- max(unlist(mclapply(seq_along(filt_chrProfilesFeature1),
  function(x) {
    max(c(filt_chrProfilesFeature1[[x]]$filt_feature), na.rm = T)
}, mc.cores = length(filt_chrProfilesFeature1))))
...
for(x in which(chrs %in% chrName)) {
  chrPartitionPlotCov2_feature(chrx = which(chrs %in% chrName),
                               ...
                               dat2 = filt_chrProfilesFeature1[[x]]$filt_feature,
                               col2 = "cyan",
                               Ylab2 = "Genes",
