...
# Usage:
# ./DNAmeth_per_scaled_win_TelCen.R DNAmeth BSseq_Rep8a_SRR6792678 1Mb 1000000 100 100ths grey50 mCG

#inDir <- "DNAmeth"
#profileName <- "BSseq_Rep8a_SRR6792678"
#winName <- "1Mb"
#winSize <- 1000000
#prop <- 100
#propName <- "100ths" 
...
profiles <- read.table(paste0("/home/ajt200/analysis/wheat/chromosomeProfiles/",
                       ...
                       header = T)
if(context == "mCG") {
  profiles <- data.frame(chr = profiles$chr,
                         window = profiles$window,
                         mCG = profiles$mCG,
                         stringsAsFactors = F)
} else if(context == "mCHG") {
  profiles <- data.frame(chr = profiles$chr,
                         window = profiles$window,
                         mCHG = profiles$mCHG,
                         stringsAsFactors = F)
} else if(context == "mCHH") {
  profiles <- data.frame(chr = profiles$chr,
                         window = profiles$window,
                         mCHH = profiles$mCHH,
                         stringsAsFactors = F)
...
write.table(TelCenMatrix,
            file = paste0(profileName, "_", context, "_",
                          winName, "_",
                          propName, "_TelCenMatrix.txt"))

# Load TelCenMatrix
TelCenDF <- read.table(paste0(profileName, "_", context, "_",
                              winName, "_",
                              propName, "_TelCenMatrix.txt"))
...
pdf(paste0(plotDir, profileName, "_", context,
           "_", winName, "_",
