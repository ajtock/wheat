...
# Usage:
# ./chrProfilePlot_1homoeologuegroup_DNAmeth_MNase_cMMb_TEsuperfam_Dan.R MNase MNase_Rep1 m_ Gypsy_LTR_RLG_smoothed both 1Mb 1000000 15 darkcyan 120520 'chr3A,chr3B,chr3D'
...
plotDir <- "/production/dh580/methylation/plots/"
...
profileDNAmeth <- read.table(paste0("/production/dh580/methylation/",
...
filt_chrProfilesDNAmeth <- mclapply(seq_along(chrProfilesDNAmeth), function(x) {
  # mCCG
  filt_chrProfile_mCCG <- stats::filter(x = chrProfilesDNAmeth[[x]]$mCCG,
                                       filter = f,
                                       sides = 2)
  filt_chrProfile_mCCG[1:flank] <- filt_chrProfile_mCCG[flank+1]
  filt_chrProfile_mCCG[(length(filt_chrProfile_mCCG)-flank+1):length(filt_chrProfile_mCCG)] <- filt_chrProfile_mCCG[(length(filt_chrProfile_mCCG)-flank)]
  # mCAG
  filt_chrProfile_mCAG <- stats::filter(x = chrProfilesDNAmeth[[x]]$mCAG,
                                        filter = f,
                                        sides = 2)
  filt_chrProfile_mCAG[1:flank] <- filt_chrProfile_mCAG[flank+1]
  filt_chrProfile_mCAG[(length(filt_chrProfile_mCAG)-flank+1):length(filt_chrProfile_mCAG)] <- filt_chrProfile_mCAG[(length(filt_chrProfile_mCAG)-flank)]
  # mCTG
  filt_chrProfile_mCTG <- stats::filter(x = chrProfilesDNAmeth[[x]]$mCTG,
                                        filter = f,
                                        sides = 2)
  filt_chrProfile_mCTG[1:flank] <- filt_chrProfile_mCTG[flank+1]
  filt_chrProfile_mCTG[(length(filt_chrProfile_mCTG)-flank+1):length(filt_chrProfile_mCTG)] <- filt_chrProfile_mCTG[(length(filt_chrProfile_mCTG)-flank)]
  # mC
  ...
  data.frame(chr = as.character(chrProfilesDNAmeth[[x]]$chr),
             ...
             filt_mCCG = as.numeric(filt_chrProfile_mCCG),
             filt_mCAG = as.numeric(filt_chrProfile_mCAG),
             filt_mCTG = as.numeric(filt_chrProfile_mCTG),
             filt_mC = as.numeric(filt_chrProfile_mC),
             stringsAsFactors = F)
}, mc.cores = length(chrProfilesDNAmeth))

min_mCCG_chrs <- min(unlist(mclapply(seq_along(filt_chrProfilesDNAmeth),
                                    function(x) {
                                      min(c(filt_chrProfilesDNAmeth[[x]]$filt_mCCG), na.rm = T)
                                    }, mc.cores = length(filt_chrProfilesDNAmeth))))
max_mCCG_chrs <- max(unlist(mclapply(seq_along(filt_chrProfilesDNAmeth),
                                    function(x) {
                                      max(c(filt_chrProfilesDNAmeth[[x]]$filt_mCCG), na.rm = T)
                                    }, mc.cores = length(filt_chrProfilesDNAmeth))))

min_mCCG <- min(unlist(mclapply(which(chrs %in% chrName),
                               function(x) {
                                 min(c(filt_chrProfilesDNAmeth[[x]]$filt_mCCG), na.rm = T)
                               }, mc.cores = length(filt_chrProfilesDNAmeth))))
max_mCCG <- max(unlist(mclapply(which(chrs %in% chrName),
                               function(x) {
                                 max(c(filt_chrProfilesDNAmeth[[x]]$filt_mCCG), na.rm = T)
                               }, mc.cores = length(filt_chrProfilesDNAmeth))))

min_mCAG_chrs <- min(unlist(mclapply(seq_along(filt_chrProfilesDNAmeth),
                                     function(x) {
                                       min(c(filt_chrProfilesDNAmeth[[x]]$filt_mCAG), na.rm = T)
                                     }, mc.cores = length(filt_chrProfilesDNAmeth))))
max_mCAG_chrs <- max(unlist(mclapply(seq_along(filt_chrProfilesDNAmeth),
                                     function(x) {
                                       max(c(filt_chrProfilesDNAmeth[[x]]$filt_mCAG), na.rm = T)
                                     }, mc.cores = length(filt_chrProfilesDNAmeth))))

min_mCAG <- min(unlist(mclapply(which(chrs %in% chrName),
                                function(x) {
                                  min(c(filt_chrProfilesDNAmeth[[x]]$filt_mCAG), na.rm = T)
                                }, mc.cores = length(filt_chrProfilesDNAmeth))))
max_mCAG <- max(unlist(mclapply(which(chrs %in% chrName),
                                function(x) {
                                  max(c(filt_chrProfilesDNAmeth[[x]]$filt_mCAG), na.rm = T)
                                }, mc.cores = length(filt_chrProfilesDNAmeth))))

min_mCTG_chrs <- min(unlist(mclapply(seq_along(filt_chrProfilesDNAmeth),
                                     function(x) {
                                       min(c(filt_chrProfilesDNAmeth[[x]]$filt_mCTG), na.rm = T)
                                     }, mc.cores = length(filt_chrProfilesDNAmeth))))
max_mCTG_chrs <- max(unlist(mclapply(seq_along(filt_chrProfilesDNAmeth),
                                     function(x) {
                                       max(c(filt_chrProfilesDNAmeth[[x]]$filt_mCTG), na.rm = T)
                                     }, mc.cores = length(filt_chrProfilesDNAmeth))))

min_mCTG <- min(unlist(mclapply(which(chrs %in% chrName),
                                function(x) {
                                  min(c(filt_chrProfilesDNAmeth[[x]]$filt_mCTG), na.rm = T)
                                }, mc.cores = length(filt_chrProfilesDNAmeth))))
max_mCTG <- max(unlist(mclapply(which(chrs %in% chrName),
                                function(x) {
                                  max(c(filt_chrProfilesDNAmeth[[x]]$filt_mCTG), na.rm = T)
                                }, mc.cores = length(filt_chrProfilesDNAmeth))))
...
pdf(paste0(plotDir, "Wheat_", paste0(chrName, collapse = "_"),
           "_mCCG_", DNAmethName,"_", libNameChIPA,
...
for(x in which(chrs %in% chrName)) {
  chrPartitionPlotCovMeth_feature2(chrx = which(chrs %in% chrName),
                                   ...
                                   dat1A = filt_chrProfilesDNAmeth[[x]]$filt_mCCG,
...
# mCAG
pdf(paste0(plotDir, "Wheat_", paste0(chrName, collapse = "_"),
           "_mCAG_", DNAmethName,"_", libNameChIPA,
           "_", superfam, "_chrPlot_winSize", winName, "_smooth", N,
           "_v", date, ".pdf"),
    height = 4, width = 10*length(chrName))
par(mfrow = c(1, length(chrName)))
...
for(x in which(chrs %in% chrName)) {
  chrPartitionPlotCovMeth_feature2(chrx = which(chrs %in% chrName),
                                   ...
                                   cenEnd = centromereEnd[x],
                                   xplot1 = filt_chrProfilesChIPA[[x]]$window,
                                   dat1A = filt_chrProfilesDNAmeth[[x]]$filt_mCAG,
...
# mCTG
pdf(paste0(plotDir, "Wheat_", paste0(chrName, collapse = "_"),
           "_mCTG_", DNAmethName,"_", libNameChIPA,
           "_", superfam, "_chrPlot_winSize", winName, "_smooth", N,
           "_v", date, ".pdf"),
    height = 4, width = 10*length(chrName))
par(mfrow = c(1, length(chrName)))
...
for(x in which(chrs %in% chrName)) {
  chrPartitionPlotCovMeth_feature2(chrx = which(chrs %in% chrName),
                                   ...
                                   cenEnd = centromereEnd[x],
                                   xplot1 = filt_chrProfilesChIPA[[x]]$window,
                                   dat1A = filt_chrProfilesDNAmeth[[x]]$filt_mCTG,
