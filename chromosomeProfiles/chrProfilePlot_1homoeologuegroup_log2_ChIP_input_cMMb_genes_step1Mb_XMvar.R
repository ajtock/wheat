...
# Usage:
# ./chrProfilePlot_1homoeologuegroup_log2_ChIP_input_cMMb_genes_step1Mb_XMvar.R DMC1 DMC1_Rep1_ChIP input H3_input_SRR6350669 1Mb 1000000 15 120421 'chr3A,chr3B,chr3D'
...
}, mc.cores = length(filt_chrProfilesChIPA))))-0.10
minCPM_chrs <- -0.4
...
for(x in which(chrs %in% chrName)) {
  chrPartitionPlotCov2l_feature2(chrx = which(chrs %in% chrName),
...
#                                 rug1Col = "grey40",
                                 ...
                                 col1A = makeTransparent("darkgreen"),
                                 dat1B = filt_chrProfilesChIPB[[x]]$filt_CPM,
                                 col1B = makeTransparent("limegreen"),
...
for(x in which(chrs %in% chrs)) {
  chrPartitionPlotCov2l_feature2(chrx = which(chrs %in% chrs),
                                 title = sub("c", "C", chrs[x]),
                                 cenStart = centromereStart[x],
                                 cenEnd = centromereEnd[x],
#                                 rug1 = markers[markers$chromosome == chrs[x],]$physicalPosition,
