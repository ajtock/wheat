...
# Usage 
# ./proportion_cluster_member_numbers_in_NLR_quantiles_hypergeometricTest.R 'cMMb' 'genes' 1 4 'genomewide' 'Agenome_Bgenome_Dgenome' 100000
...
# Define cluster member numbers
clSizes <- sort(unique(featuresDF$cluster_members))
...
for(yy in seq_along(clSizes)) {
  ...
  # (this excludes genome_clSize genes not assigned to a chromosome)
  genome_clSize <- intersect(genome_clSize, genome_genes)
  ...
  # length(quantile_clSize) to length(quantile_genes)
  ...
  for(z in seq_along(quantile_genes_list)) {
    ...
    quantile_clSize <- intersect(quantile_genes, genome_clSize)
  
    # Calculate the P-values for over-representation and under-representation
    # of NLRs among quantile z genes
    set.seed(2847502)
    # Over-representation:
    Pval_overrep <- sum(dhyper(x = length(quantile_clSize):length(quantile_genes),
                               m = length(genome_clSize),
                               n = length(genome_genes) - length(genome_clSize),
                               k = length(quantile_genes)))
    print(Pval_overrep)
  
    # Or by 1 minus the sum of the probabilities of drawing 0:(length(quantile_clSize)-1)
    print(1 - sum(dhyper(x = 0:(length(quantile_clSize)-1),
                         m = length(genome_clSize),
                         n = length(genome_genes) - length(genome_clSize),
                         k = length(quantile_genes))))
  
    # Under-representation
    Pval_underrep <- phyper(q = length(quantile_clSize),
                            m = length(genome_clSize),
                            n = length(genome_genes) - length(genome_clSize),
                            k = length(quantile_genes))
    print(Pval_underrep)
    ...
    if(length(quantile_clSize) > mean(hgDist)) {
      Pval <- Pval_overrep
    ...
    hgTestResults <- new("hypergeomTest",
                         ...
                         alpha0.05 = alpha0.05,
                         pval = Pval,
                         observed = length(quantile_clSize),
                         expected = mean(hgDist),
                         log2obsexp = log2( (length(quantile_clSize)+1) / (mean(hgDist)+1) ),
                         log2alpha  = log2( (alpha0.05+1) / (mean(hgDist)+1) ),
                         quantile_genes = length(quantile_genes),
                         proportion_of_quantile = length(quantile_clSize) / length(quantile_genes),
