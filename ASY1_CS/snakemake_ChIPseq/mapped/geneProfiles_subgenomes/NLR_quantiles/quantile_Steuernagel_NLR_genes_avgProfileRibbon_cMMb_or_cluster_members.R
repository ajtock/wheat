...
# Plot average coverage profiles with 95% CIs around gene quantiles

# Usage:
# /applications/R/R-3.5.0/bin/Rscript quantile_Steuernagel_NLR_genes_avgProfileRibbon_cMMb_or_cluster_members.R cMMb both 'NLR_genes_in_Agenome_genomewide,NLR_genes_in_Bgenome_genomewide,NLR_genes_in_Dgenome_genomewide' 3500 2000 2kb '2 kb' 20 20bp genes 4 100kb 1 '0.02,0.96'
...
control_featureMats <- mclapply(seq_along(controlNames), function(x) {
  lapply(seq_along(featureName), function(y) {
    as.matrix(read.table(paste0(controlDirs[x],
                                ...
                                substring(featureName[y], first = 5), "_matrix_bin", binName,
...
control_ranLocMats <- mclapply(seq_along(controlNames), function(x) {
  lapply(seq_along(featureName), function(y) {
    as.matrix(read.table(paste0(controlDirs[x],
                                controlNames[x],
                                "_MappedOn_wheat_v1.0_lowXM_", align, "_sort_norm_",
                                substring(featureName[y], first = 5), "_ranLoc_matrix_bin", binName,
                                "_flank", flankName, ".tab"),
                         header = F, skip = 3))
  })
}, mc.cores = length(controlNames))
...
SNPclass_featureMats <- mclapply(seq_along(SNPclassNames), function(x) {
  lapply(seq_along(featureName), function(y) {
    as.matrix(read.table(paste0("/home/ajt200/analysis/wheat/ASY1_CS/snakemake_ChIPseq/mapped/geneProfiles_subgenomes/matrices/",
                                ...
                                "_around_", substring(featureName[y], first = 5),
...
SNPclass_featureMats <- mclapply(seq_along(SNPclassNames), function(x) {
  lapply(seq_along(featureName), function(y) {
    as.matrix(read.table(paste0("/home/ajt200/analysis/wheat/ASY1_CS/snakemake_ChIPseq/mapped/geneProfiles_subgenomes/matrices/exome_",
                                SNPclassNames[x],
                                "_SNPs_around_", substring(featureName[y], first = 5),
                                "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = T))
  })
}, mc.cores = length(SNPclassNames))
