...
# Usage
# ./proportion_motif_counts_in_NLR_quantiles_hypergeometricTest_bargraph_only.R 'cMMb' 'genes' 1 4 'genomewide' 100000 'black,navy,dodgerblue4,deepskyblue'
...
featCat <- paste0("NLR_motif_", c(1:20))
featCatPlot <- paste0("NLR motif ", c(1:20), " counts")
...
for(yy in seq_along(featCat)) {
  hg_list <- lapply(seq_along(genomeNames), function(y) {
    ...
    for(z in quantileFirst:quantileLast) {
      if(libName %in% c("cMMb", "HudsonRM_all")) {
      load(paste0(outDir,
                  featCat[yy], "_count_representation_among_quantile", z, "_of_", quantileLast,
  ...
  ggsave(paste0(plotDir,
                "combined_bargraph_", featCat[yy], "_count_representation_among_", quantileLast,
