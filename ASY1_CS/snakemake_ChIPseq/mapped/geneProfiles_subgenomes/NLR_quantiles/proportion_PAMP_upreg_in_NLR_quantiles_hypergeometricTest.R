...
# Usage 
# ./proportion_PAMP_upreg_in_NLR_quantiles_hypergeometricTest.R 'cMMb' 'genes' 1 4 'genomewide' 'Agenome_Bgenome_Dgenome' 100000
...
PAMP_upreg_NLRs <- read.table(paste0("/home/ajt200/analysis/wheat/annotation/221118_download/iwgsc_refseqv1.1_genes_2017July06/",
                                     "NLRs_Steuernagel_Wulff_2020_Plant_Physiol/NLR_genes_upregulated_PAMP_treatment_representative_mRNA.gff3"),
                              header = F, stringsAsFactors = F)
PAMP_upreg_NLRs$V9 <- sub(pattern = "\\.\\d+", replacement = "",
                          x = PAMP_upreg_NLRs$V9)
...
# Get IDs of NLR genes that are upregulated under PAMP treatment
genome_upreg <- PAMP_upreg_NLRs$V9
...
#    textX1 <- min(hgTestResults@hypergeomDist)/1.15
  ...
  mtext(side = 1,
        text = bquote("Genes"),
        line = 1.85)
  titleText <- list(bquote("NLRs upregulated under PAMP in" ~
...
#  scale_y_continuous(limits = c(-1.5, 1.5)) +
  ...
  ggtitle(bquote("NLRs upregulated under PAMP in" ~
