...
colourB <- args[14]
date <- args[15]
chrName <- unlist(strsplit(args[16],
                           split = ","))
libName <- args[17]
featureName <- unlist(strsplit(args[18],
                               split = ","))
region <- args[19]
quantiles <- as.numeric(args[20])
...
# Define quantile colours and names
quantileColours <- c("red", "purple", "blue", "navy")
quantileNames <- paste0("Quantile ", 1:quantiles)
...
featureA <- read.table(paste0("/home/ajt200/analysis/wheat/chromosomeProfiles/cMMb/cMMb_iwgsc_refseqv1.0_mapping_data_minInterMarkerDist",
                       ...
                       header = T)
if(libName %in% c("cMMb", "cluster_members")) {
  featureB <- read.table(paste0(outDir,
...
minFeatureB_chrs <- min(unlist(mclapply(seq_along(filt_chrProfilesFeatureB),
  function(x) {
    min(c(filt_chrProfilesFeatureB[[x]][,3],
          filt_chrProfilesFeatureB[[x]][,4],
          filt_chrProfilesFeatureB[[x]][,5],
          filt_chrProfilesFeatureB[[x]][,6]), na.rm = T)
}, mc.cores = length(filt_chrProfilesFeatureB))))
maxFeatureB_chrs <- max(unlist(mclapply(seq_along(filt_chrProfilesFeatureB),
  function(x) {
    max(c(filt_chrProfilesFeatureB[[x]][,3],
          filt_chrProfilesFeatureB[[x]][,4],
          filt_chrProfilesFeatureB[[x]][,5],
          filt_chrProfilesFeatureB[[x]][,6]), na.rm = T)
}, mc.cores = length(filt_chrProfilesFeatureB))))
...
if(libName %in% c("cMMb", "cluster_members")) {
  pdf(paste0(plotDir, "Wheat_", paste0(chrName, collapse = "_"),
             ...
             align, "_cMMb_and_NLR_quantiles",
             "_by_", libName, "_of_",
             substring(featureName[1][1], first = 1, last = 9),
             "_v", date, ".pdf"),
...
for(x in which(chrs %in% chrName)) {
  chrPartitionPlotCov2_featureQuantiles(chrx = which(chrs %in% chrName),
                                        title = paste0(sub("c", "C", chrs[x]), "        ", featureNamePlot),
...
for(x in which(chrs %in% chrs)) {
  chrPartitionPlotCov2_featureQuantiles(chrx = which(chrs %in% chrs),
                                        title = paste0(sub("c", "C", chrs[x]), "        ", featureNamePlot),
                                        cenStart = centromereStart[x],
                                        cenEnd = centromereEnd[x],
                                        rug1 = markers[markers$chromosome == chrs[x],]$physicalPosition,
