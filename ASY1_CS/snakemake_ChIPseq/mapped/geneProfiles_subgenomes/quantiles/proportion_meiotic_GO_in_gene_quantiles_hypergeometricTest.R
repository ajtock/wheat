...
# Meiotic-GO genes (as defined by Ksenia Krasileva) and, separately,
# meiotic-module genes (as defined in Alabdullah et al. 2019 Front. Plant Sci.)
# (i.e., is the propotion of Meiotic-GO genes or meiotic-module genes
...
# Usage 
# ./proportion_meiotic_GO_in_gene_quantiles_hypergeometricTest.R 'ASY1_CS_Rep1_ChIP' 'bodies' 1 4 'genomewide' 'Agenome_Bgenome_Dgenome' 100000
...
meio <- read.table("/home/ajt200/analysis/wheat/RNAseq_meiocyte_Alabdullah_Moore_2019_FrontPlantSci/Table_S4_meiotic_GO_genes.tsv",
                   header = T, stringsAsFactors = F)
genome_meio <- as.character(meio$Gene.ID)
#meio <- read.table("/home/ajt200/analysis/wheat/RNAseq_meiocyte_Alabdullah_Moore_2019_FrontPlantSci/Table_S4_meiotic_gene_orthologs.tsv",
...
for(z in seq_along(quantile_genes_list)) {
  ...
  save(hgTestResults,
       file = paste0(outDir,
                     "meiotic_GO_gene_representation_among_quantile", z, "_of_", quantileLast,
...
#    textX1 <- min(hgTestResults@hypergeomDist)/1.15
  ...
  mtext(side = 1,
        text = bquote("Genes"),
        line = 1.85)
  titleText <- list(bquote("Meiotic-GO genes in" ~
...
#  scale_y_continuous(limits = c(-1.5, 1.5)) +
  ...
  ggtitle(bquote("Meiotic-GO genes in" ~
...
ggsave(paste0(plotDir,
              "bargraph_meiotic_GO_gene_representation_among_", quantileLast,
