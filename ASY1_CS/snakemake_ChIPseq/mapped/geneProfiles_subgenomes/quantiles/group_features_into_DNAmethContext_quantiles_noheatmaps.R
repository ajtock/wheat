...
# Usage:
# /applications/R/R-3.4.0/bin/Rscript group_features_into_DNAmethContext_quantiles_noheatmaps.R BSseq_Rep8a_SRR6792678 'epigenomics_shoot_leaf_IWGSC_2018_Science/BSseq' both 'genes_in_Agenome_genomewide,genes_in_Bgenome_genomewide,genes_in_Dgenome_genomewide' 3500 2000 2kb '2 kb' 20 20bp bodies

#libName <- "BSseq_Rep8a_SRR6792678"
#dirName <- "epigenomics_shoot_leaf_IWGSC_2018_Science/BSseq"
#context <- "CpG"
...
outDir <- paste0("quantiles_by_DNAmeth_", context,
                 "_in_", region, "/")
plotDir <- paste0(outDir, "plots/")
system(paste0("[ -d ", outDir, " ] || mkdir ", outDir))
system(paste0("[ -d ", plotDir, " ] || mkdir ", plotDir))

# Load matrix of windowed DNA methylation context proportions
mat1 <- lapply(seq_along(featureName), function(y) {
  as.matrix(read.table(paste0("/home/ajt200/analysis/wheat/",
                              ...
                              libName,
                              "_MappedOn_wheat_v1.0_incl_organelles_controls_dedup_", context, "_",
...
if( region == "promoters" ) {
  ...
  mat1Region <- mat1[,(((upstream-1000)/binSize)+1):(((upstream+bodyLength)/binSize)+(1000/binSize))]
} else {
...
mat1RegionRowMeansSorted <- sort.int(mat1RegionRowMeans,
                                     ...
                                     index.return = T,
                                     na.last = T)
mat1RegionSorted <- mat1Region[sort.int(mat1RegionRowMeans,
                                        decreasing = T,
                                        index.return = T,
                                        na.last = T)$ix,]
...
# Combine feature coordinates and their corresponding mean coverage values,
# cM/Mb values, and intron and exon numbers
featuresGR <- GRanges(featuresGR,
                      featureID = featuresGR$featureID,
                      mat1RegionRowMeans = mat1RegionRowMeans,
                      cMMb = feature_cMMb,
                      exons = exonNoPerGene,
...
# Divide features into quantiles based on decreasing mat1RegionRowMeans
...
for(k in 1:quantiles) {
  ...
  # First quantile should span 1 to greater than, e.g., 0.75 proportions of features
    featuresDF[ !is.na(featuresDF$mat1RegionRowMeans) &
                percent_rank(featuresDF$mat1RegionRowMeans) <= 1-((k-1)/quantiles) &
                percent_rank(featuresDF$mat1RegionRowMeans) >  1-(k/quantiles), ]$quantile <- paste0("Quantile ", k)
  } else {
  ...
  write.table(featuresDF[featuresDF$quantile == paste0("Quantile ", k),],
              file = paste0(outDir,
                            ...
                            "_by_", libName, "_DNAmeth_", context, "_in_",
                            region, "_of_",
  ...
  stats <- data.frame(quantile = as.integer(k),
                      ...
                      mean_mat1RegionRowMeans = as.numeric(mean(featuresDF[featuresDF$quantile == paste0("Quantile ", k),]$mat1RegionRowMeans, na.rm = T)))
  quantilesStats <- rbind(quantilesStats, stats)
}
write.table(quantilesStats,
            file = paste0(outDir,
                          "summary_", quantiles, "quantiles_by_", libName, "_DNAmeth_", context, "_in_",
                          region, "_of_",
                          substring(featureName[1][1], first = 1, last = 5), "_in_",
                          paste0(substring(featureName, first = 10, last = 16),
                                 collapse = "_"), "_",
...
# Order features in each quantile by decreasing mat1Region levels
...
combineRowOrders <- function(quantile_bool_list) {
  do.call("c", lapply(quantile_bool_list, function(x) {
    quantile_mat1RegionRowMeans <- rowMeans(mat1Region[x,], na.rm = T)
    which(x)[order(quantile_mat1RegionRowMeans, decreasing = T)]
  }))
}
...
stopifnot(identical(row_order,
                    order(featuresDF$mat1RegionRowMeans,
                          decreasing=T)))

# Order feature IDs in each quantile by decreasing mat1Region levels
...
#mat1s <- mclapply(seq_along(ChIPmats), function(x) {
#  if(ChIPNames[x] %in% c(
#                         "ASY1_CS_Rep1_ChIP",
#                         "DMC1_Rep1_ChIP",
...
#}, mc.cores = length(ChIPmats))
#
#for(x in seq_along(mat1s)) {
#  attr(mat1s[[x]], "upstream_index") = 1:(upstream/binSize)
#  attr(mat1s[[x]], "target_index") = ((upstream/binSize)+1):((upstream+bodyLength)/binSize)
#  attr(mat1s[[x]], "downstream_index") = (((upstream+bodyLength)/binSize)+1):(((upstream+bodyLength)/binSize)+(downstream/binSize))
#  attr(mat1s[[x]], "extend") = c(upstream, downstream)
#  attr(mat1s[[x]], "smooth") = FALSE
#  attr(mat1s[[x]], "signal_name") = ChIPNamesPlot[x]
#  attr(mat1s[[x]], "target_name") = featureName
#  attr(mat1s[[x]], "target_is_single_point") = FALSE
#  attr(mat1s[[x]], "background") = 0
#  attr(mat1s[[x]], "signal_is_categorical") = FALSE
#  class(mat1s[[x]]) = c("normalizedMatrix", "matrix")
#}
#
