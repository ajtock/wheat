...
# P-value is the probability of drawing >= length(quantile_ASRs) [x] features
# in a sample size of length(quantile_genes) [k] from a total feature set consisting of
# length(genome_ASRs) [m] + ( length(genome_genes) - length(genome_ASRs)) [n]

# Usage 
# ./proportion_abiotic_stress_responsive_genes_in_gene_quantiles_hypergeometricTest.R 'ASY1_CS_Rep1_ChIP' 'genes' 1 4 'genomewide' 'Agenome_Bgenome_Dgenome' 100000
...
stress <- c("*", stress)
stressName <- sub("*", "abiotic_stress", stress, fixed = T)

mclapply(seq_along(stress), function(xx) {
  print(stressName[[xx]])
  ASRs_stress <- ASRs[grepl(stress[[xx]], ASRs$stress),]
  print(dim(ASRs_stress))
  genome_ASRs <- ASRs_stress$gene
  
  # Subset ASRs to only those within a given subgenome
  if(length(genomeLetter) == 1) {
    genome_ASRs <- genome_ASRs[grepl(paste0("TraesCS\\d", genomeLetter, "02G"), genome_ASRs)]
  }
  
  genome_ASRs <- intersect(genome_ASRs, genome_genes)
  ...
  # length(quantile_ASRs) to length(quantile_genes)
  ...
  for(z in seq_along(quantile_genes_list)) {
    ...
    # Get intersection of gene IDs in quantile z and gene IDs of ASRs
    quantile_ASRs <- intersect(quantile_genes, genome_ASRs)
  
    # Calculate the P-values for over-representation and under-representation
    # of ASRs among quantile z genes
    set.seed(2847502)
    # Over-representation:
    Pval_overrep <- sum(dhyper(x = length(quantile_ASRs):length(quantile_genes),
                               m = length(genome_ASRs),
                               n = length(genome_genes) - length(genome_ASRs),
                               k = length(quantile_genes)))
    print(Pval_overrep)
  
    # Or by 1 minus the sum of the probabilities of drawing 0:(length(quantile_ASRs)-1)
    print(1 - sum(dhyper(x = 0:(length(quantile_ASRs)-1),
                         m = length(genome_ASRs),
                         n = length(genome_genes) - length(genome_ASRs),
                         k = length(quantile_genes))))
  
    # Under-representation
    Pval_underrep <- phyper(q = length(quantile_ASRs),
                            m = length(genome_ASRs),
                            n = length(genome_genes) - length(genome_ASRs),
                            k = length(quantile_genes))
    print(Pval_underrep)
    ...
    if(length(quantile_ASRs) > mean(hgDist)) {
      Pval <- Pval_overrep
    ...
    hgTestResults <- new("hypergeomTest",
                         ...
                         alpha0.05 = alpha0.05,
                         pval = Pval,
                         observed = length(quantile_ASRs),
                         expected = mean(hgDist),
                         log2obsexp = log2( length(quantile_ASRs) / mean(hgDist) ),
                         log2alpha  = log2( alpha0.05 / mean(hgDist) ),
                         quantile_genes = length(quantile_genes),
                         proportion_of_quantile = length(quantile_ASRs) / length(quantile_genes),
                         random_proportions_of_quantile = hgDist / length(quantile_genes),
                         hypergeomDist = hgDist)
    if(libName %in% "cMMb") {
    save(hgTestResults,
         file = paste0(outDir,
                       stressName[xx], "_ASR_gene_representation_among_quantile", z, "_of_", quantileLast,
  ...
  ggsave(paste0(plotDir,
                "bargraph_", stressName[xx], "_ASR_gene_representation_among_", quantileLast,
