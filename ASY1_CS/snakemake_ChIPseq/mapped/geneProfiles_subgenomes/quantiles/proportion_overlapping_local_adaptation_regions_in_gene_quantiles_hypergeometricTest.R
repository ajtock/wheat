...
# genes overlapping selective sweep regions associated with local_adaptation
...
# P-value is the probability of drawing >= length(quantile_LARs) [x] features
...
# Load selective sweep regions assocaited with local_adaptation (LARs)
...
LARs <- GRanges(seqnames = LARs$chr,
                ranges = IRanges(start = LARs$start,
                                 end = LARs$end),
                strand = "*")
LARs_genome_genes_overlap <- findOverlaps(query = LARs,
...
# and genome_LARs are LAR-overlapping genes across all quantiles
...
# length(quantile_LARs) to length(quantile_genes)
...
for(z in seq_along(quantile_genes_list)) {
  ...
  set.seed(2847502)
  # Over-representation:
  Pval_overrep <- sum(dhyper(x = length(quantile_LARs):length(quantile_genes),
                             m = length(genome_LARs),
                             n = length(genome_genes) - length(genome_LARs),
                             k = length(quantile_genes)))
  print(Pval_overrep)

  # Or by 1 minus the sum of the probabilities of drawing 0:(length(quantile_LARs)-1)
  print(1 - sum(dhyper(x = 0:(length(quantile_LARs)-1),
                       m = length(genome_LARs),
                       n = length(genome_genes) - length(genome_LARs),
                       k = length(quantile_genes))))

  # Under-representation
  Pval_underrep <- phyper(q = length(quantile_LARs),
                          m = length(genome_LARs),
                          n = length(genome_genes) - length(genome_LARs),
                          k = length(quantile_genes))
  print(Pval_underrep)
  ...
  if(length(quantile_LARs) > mean(hgDist)) {
    Pval <- Pval_overrep
  ...
  hgTestResults <- new("hypergeomTest",
                       ...
                       alpha0.05 = alpha0.05,
                       pval = Pval,
                       observed = length(quantile_LARs),
                       expected = mean(hgDist),
                       log2obsexp = log2( length(quantile_LARs) / mean(hgDist) ),
                       log2alpha  = log2( alpha0.05 / mean(hgDist) ),
                       quantile_genes = length(quantile_genes),
                       proportion_of_quantile = length(quantile_LARs) / length(quantile_genes),
                       random_proportions_of_quantile = hgDist / length(quantile_genes),
                       hypergeomDist = hgDist)
  if(libName %in% "cMMb") {
  save(hgTestResults,
       file = paste0(outDir,
                     "LAR_overlapping_gene_representation_among_quantile", z, "_of_", quantileLast,
...
#    textX1 <- min(hgTestResults@hypergeomDist)/1.15
  ...
  mtext(side = 1,
        text = bquote("Genes"),
        line = 1.85)
  titleText <- list(bquote("LAR-overlapping genes in" ~
...
#  scale_y_continuous(limits = c(-1.5, 1.5)) +
  ...
  ggtitle(bquote("LAR-overlapping genes in" ~
...
ggsave(paste0(plotDir,
              "bargraph_LAR_overlapping_gene_representation_among_", quantileLast,
