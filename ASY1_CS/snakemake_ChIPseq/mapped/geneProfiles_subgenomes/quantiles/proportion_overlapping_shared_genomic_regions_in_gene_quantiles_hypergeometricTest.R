...
# P-value is the probability of drawing >= length(quantile_SGRs) [x] features
# in a sample size of length(quantile_genes) [k] from a total feature set consisting of
# length(genome_SGRs) [m] + ( length(genome_genes) - length(genome_SGRs)) [n]

# Usage 
# ./proportion_overlapping_shared_genomic_regions_in_gene_quantiles_hypergeometricTest.R 'ASY1_CS_Rep1_ChIP' 'bodies' 1 4 'genomewide' 'Agenome' 100000
...
# Load shared genomic regions (SGRs)
SGRs <- read.table(paste0("/home/ajt200/analysis/wheat/annotation/221118_download/He_Akhunov_2019_NatGenet_1000exomes_SNPs/",
                          "Table_S12_41588_2019_382_MOESM12_ESM.tsv"),
...
# Subset SGRs to only those within a given subgenome
if(length(genomeLetter) == 1) {
  chrs <- chrs[grepl(genomeLetter, chrs)]
  SGRs <- SGRs[SGRs$chr %in% chrs,]
}
SGRs <- GRanges(seqnames = SGRs$chr,
                ranges = IRanges(start = SGRs$start,
                                 end = SGRs$end),
                strand = "*")
SGRs_genome_genes_overlap <- findOverlaps(query = SGRs,
                                          subject = genome_genes,
                                          type = "any",
                                          select = "all",
                                          ignore.strand = T)
genome_SGRs <- genome_genes[unique(subjectHits(SGRs_genome_genes_overlap))]
...
# in a sample size of length(quantile_genes) [k] from a total feature set consisting of
# length(genome_SGRs) [m] + ( length(genome_genes) - length(genome_SGRs)) [n] ,
# where quantile_SGRs are SGR-overlapping genes in a given quantile,
# and genome_SGRs are SGR-overlapping genes across all quantiles
...
# length(quantile_SGRs) to length(quantile_genes)
...
for(z in seq_along(quantile_genes_list)) {
  ...
  # Within the given quantile, get genes that overlap SGRs
  quantile_SGRs <- genome_SGRs[genome_SGRs$quantile == paste0("Quantile ", z)]

  # Calculate the P-values for over-representation and under-representation
  # of SGRs among quantile z genes
  set.seed(2847502)
  # Over-representation:
  Pval_overrep <- sum(dhyper(x = length(quantile_SGRs):length(quantile_genes),
                             m = length(genome_SGRs),
                             n = length(genome_genes) - length(genome_SGRs),
                             k = length(quantile_genes)))
  print(Pval_overrep)

  # Or by 1 minus the sum of the probabilities of drawing 0:(length(quantile_SGRs)-1)
  print(1 - sum(dhyper(x = 0:(length(quantile_SGRs)-1),
                       m = length(genome_SGRs),
                       n = length(genome_genes) - length(genome_SGRs),
                       k = length(quantile_genes))))

  # Under-representation
  Pval_underrep <- phyper(q = length(quantile_SGRs),
                          m = length(genome_SGRs),
                          n = length(genome_genes) - length(genome_SGRs),
                          k = length(quantile_genes))
  print(Pval_underrep)
  ...
  if(length(quantile_SGRs) > mean(hgDist)) {
    Pval <- Pval_overrep
  ...
  hgTestResults <- new("hypergeomTest",
                       ...
                       alpha0.05 = alpha0.05,
                       pval = Pval,
                       observed = length(quantile_SGRs),
                       expected = mean(hgDist),
                       log2obsexp = log2( length(quantile_SGRs) / mean(hgDist) ),
                       log2alpha  = log2( alpha0.05 / mean(hgDist) ),
                       quantile_genes = length(quantile_genes),
                       proportion_of_quantile = length(quantile_SGRs) / length(quantile_genes),
                       random_proportions_of_quantile = hgDist / length(quantile_genes),
                       hypergeomDist = hgDist)
  if(libName %in% "cMMb") {
  save(hgTestResults,
       file = paste0(outDir,
                     "SGR_overlapping_gene_representation_among_quantile", z, "_of_", quantileLast,
...
#    textX1 <- min(hgTestResults@hypergeomDist)/1.15
  ...
  mtext(side = 1,
        text = bquote("Genes"),
        line = 1.85)
  titleText <- list(bquote("SGR-overlapping genes in" ~
...
#  scale_y_continuous(limits = c(-1.5, 1.5)) +
  ...
  ggtitle(bquote("SGR-overlapping genes in" ~
...
ggsave(paste0(plotDir,
              "bargraph_SGR_overlapping_gene_representation_among_", quantileLast,
