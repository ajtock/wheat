...
# genes overlapping introgressed genomic regions
# (IGR-overlapping genes) (regions as identified by He et al. 2019 Nat. Genet. 51)
# (i.e., is the propotion of IGR-overlapping genes
# contained within each gene quantile significantly greater or smaller than expected by chance
# based on the hypergeometric distribution?)

# P-value is the probability of drawing >= length(quantile_IGRs) [x] features
# in a sample size of length(quantile_genes) [k] from a total feature set consisting of
# length(genome_IGRs) [m] + ( length(genome_genes) - length(genome_IGRs)) [n]

# Usage 
# ./proportion_overlapping_introgressed_genomic_regions_in_gene_quantiles_hypergeometricTest.R 'ASY1_CS_Rep1_ChIP' 'bodies' 1 4 'genomewide' 'Agenome' 100000
...
# Load introgressed genomic regions (IGRs)
IGRs <- read.table(paste0("/home/ajt200/analysis/wheat/annotation/221118_download/He_Akhunov_2019_NatGenet_1000exomes_SNPs/",
                          "Table_S8_41588_2019_382_MOESM8_ESM.tsv"),
...
# Subset IGRs to only those within a given subgenome
if(length(genomeLetter) == 1) {
  chrs <- chrs[grepl(genomeLetter, chrs)]
  IGRs <- IGRs[IGRs$chr %in% chrs,]
}
IGRs <- GRanges(seqnames = IGRs$chr,
                ranges = IRanges(start = IGRs$start,
                                 end = IGRs$end),
                strand = "*")
IGRs_genome_genes_overlap <- findOverlaps(query = IGRs,
                                          subject = genome_genes,
                                          type = "any",
                                          select = "all",
                                          ignore.strand = T)
genome_IGRs <- genome_genes[unique(subjectHits(IGRs_genome_genes_overlap))]
...
# in a sample size of length(quantile_genes) [k] from a total feature set consisting of
# length(genome_IGRs) [m] + ( length(genome_genes) - length(genome_IGRs)) [n] ,
# where quantile_IGRs are IGR-overlapping genes in a given quantile,
# and genome_IGRs are IGR-overlapping genes across all quantiles
...
# length(quantile_IGRs) to length(quantile_genes)
...
for(z in seq_along(quantile_genes_list)) {
  ...
  # Within the given quantile, get genes that overlap IGRs
  quantile_IGRs <- genome_IGRs[genome_IGRs$quantile == paste0("Quantile ", z)]

  # Calculate the P-values for over-representation and under-representation
  # of IGRs among quantile z genes
  set.seed(2847502)
  # Over-representation:
  Pval_overrep <- sum(dhyper(x = length(quantile_IGRs):length(quantile_genes),
                             m = length(genome_IGRs),
                             n = length(genome_genes) - length(genome_IGRs),
                             k = length(quantile_genes)))
  print(Pval_overrep)

  # Or by 1 minus the sum of the probabilities of drawing 0:(length(quantile_IGRs)-1)
  print(1 - sum(dhyper(x = 0:(length(quantile_IGRs)-1),
                       m = length(genome_IGRs),
                       n = length(genome_genes) - length(genome_IGRs),
                       k = length(quantile_genes))))

  # Under-representation
  Pval_underrep <- phyper(q = length(quantile_IGRs),
                          m = length(genome_IGRs),
                          n = length(genome_genes) - length(genome_IGRs),
                          k = length(quantile_genes))
  print(Pval_underrep)
  ...
  if(length(quantile_IGRs) > mean(hgDist)) {
    Pval <- Pval_overrep
  ...
  hgTestResults <- new("hypergeomTest",
                       ...
                       alpha0.05 = alpha0.05,
                       pval = Pval,
                       observed = length(quantile_IGRs),
                       expected = mean(hgDist),
                       log2obsexp = log2( length(quantile_IGRs) / mean(hgDist) ),
                       log2alpha  = log2( alpha0.05 / mean(hgDist) ),
                       quantile_genes = length(quantile_genes),
                       proportion_of_quantile = length(quantile_IGRs) / length(quantile_genes),
                       random_proportions_of_quantile = hgDist / length(quantile_genes),
                       hypergeomDist = hgDist)
  if(libName %in% "cMMb") {
  save(hgTestResults,
       file = paste0(outDir,
                     "IGR_overlapping_gene_representation_among_quantile", z, "_of_", quantileLast,
...
#    textX1 <- min(hgTestResults@hypergeomDist)/1.15
  ...
  mtext(side = 1,
        text = bquote("Genes"),
        line = 1.85)
  titleText <- list(bquote("IGR-overlapping genes in" ~
...
#  scale_y_continuous(limits = c(-1.5, 1.5)) +
  ...
  ggtitle(bquote("IGR-overlapping genes in" ~
...
ggsave(paste0(plotDir,
              "bargraph_IGR_overlapping_gene_representation_among_", quantileLast,
