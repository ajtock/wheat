...
# Usage:
# /applications/R/R-3.5.0/bin/Rscript quantile_GOann_genes_popgen_stats.R ASY1_CS_Rep1_ChIP ASY1_CS 'genes_in_Agenome_genomewide,genes_in_Bgenome_genomewide,genes_in_Dgenome_genomewide' 'Defense_response_genes' '0006952' promoters 1 4 TajimaD 'Tajima D' 10000 0.0001
...
if(IDs_mean < mean(ran_nonIDs_perm_means)) {
  nonIDs_pval <- 1 - ( sum(IDs_lessThan_nonIDs_Bool) / randomSets )
  if(nonIDs_pval == 0) {
    nonIDs_pval <- minPval
  }
  ...
  nonIDs_pval <- 1 - ( sum(IDs_moreThan_nonIDs_Bool) / randomSets )
  if(nonIDs_pval == 0) {
    nonIDs_pval <- minPval
  }
...
if(nonIDs_permTestResults@alternative == "MoreThanRandom") {
  xlim <- c(pmin(10, min(nonIDs_permTestResults@permuted)/1.2),
            pmax(nonIDs_permTestResults@observed*1.2, nonIDs_permTestResults@alpha0.05*1.2))
  textX1 <- pmax(0.05, min(nonIDs_permTestResults@permuted)/1.15)
} else {
  xlim <- c(pmin(-10, nonIDs_permTestResults@observed/1.2),
            max(nonIDs_permTestResults@permuted)*1.2)
  textX1 <- max(nonIDs_permTestResults@permuted)*1.15
}
...
if(IDs_mean < mean(ran_annoGOIDs_perm_means)) {
  annoGOIDs_pval <- 1 - ( sum(IDs_lessThan_annoGOIDs_Bool) / randomSets )
  if(annoGOIDs_pval == 0) {
  ...
  annoGOIDs_pval <- 1 - ( sum(IDs_moreThan_annoGOIDs_Bool) / randomSets )
  if(annoGOIDs_pval == 0) {
    annoGOIDs_pval <- minPval
  }
...
if(annoGOIDs_permTestResults@alternative == "MoreThanRandom") {
  xlim <- c(pmin(-10, min(annoGOIDs_permTestResults@permuted)/1.2),
            pmax(annoGOIDs_permTestResults@observed*1.2, annoGOIDs_permTestResults@alpha0.05*1.2))
  textX1 <- pmax(0.05, min(annoGOIDs_permTestResults@permuted)/1.15)
} else {
  xlim <- c(pmin(-10, annoGOIDs_permTestResults@observed/1.2),
            max(annoGOIDs_permTestResults@permuted)*1.2)
  textX1 <- max(annoGOIDs_permTestResults@permuted)*1.15
}
