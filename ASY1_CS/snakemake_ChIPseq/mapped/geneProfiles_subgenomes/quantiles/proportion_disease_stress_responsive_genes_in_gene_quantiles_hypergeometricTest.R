...
# disease-stress-responsive (DSR) genes (as defined by RamirezGonzalez ... Uauy 2018 Science)
# (i.e., is the proportion of DSR genes
# contained within each gene quantile significantly greater or smaller than expected by chance
# based on the hypergeometric distribution?)

# P-value is the probability of drawing >= length(quantile_DSRs) [x] features
# in a sample size of length(quantile_genes) [k] from a total feature set consisting of
# length(genome_DSRs) [m] + ( length(genome_genes) - length(genome_DSRs)) [n]

# Usage 
# ./proportion_disease_stress_responsive_genes_in_gene_quantiles_hypergeometricTest.R 'ASY1_CS_Rep1_ChIP' 'genes' 1 4 'genomewide' 'Agenome_Bgenome_Dgenome' 100000
...
mclapply(seq_along(stress), function(xx) {
  print(stressName[[xx]])
  DSRs_stress <- DSRs[grepl(stress[[xx]], DSRs$stress),]
  print(dim(DSRs_stress))
  genome_DSRs <- DSRs_stress$gene
  
  # Subset DSRs to only those within a given subgenome
  if(length(genomeLetter) == 1) {
    genome_DSRs <- genome_DSRs[grepl(paste0("TraesCS\\d", genomeLetter, "02G"), genome_DSRs)]
  }
  
  genome_DSRs <- intersect(genome_DSRs, genome_genes)
  ...
  # length(quantile_DSRs) to length(quantile_genes)
  ...
  for(z in seq_along(quantile_genes_list)) {
    ...
    # Get intersection of gene IDs in quantile z and gene IDs of DSRs
    quantile_DSRs <- intersect(quantile_genes, genome_DSRs)
  
    # Calculate the P-values for over-representation and under-representation
    # of DSRs among quantile z genes
    set.seed(2847502)
    # Over-representation:
    Pval_overrep <- sum(dhyper(x = length(quantile_DSRs):length(quantile_genes),
                               m = length(genome_DSRs),
                               n = length(genome_genes) - length(genome_DSRs),
                               k = length(quantile_genes)))
    print(Pval_overrep)
  
    # Or by 1 minus the sum of the probabilities of drawing 0:(length(quantile_DSRs)-1)
    print(1 - sum(dhyper(x = 0:(length(quantile_DSRs)-1),
                         m = length(genome_DSRs),
                         n = length(genome_genes) - length(genome_DSRs),
                         k = length(quantile_genes))))
  
    # Under-representation
    Pval_underrep <- phyper(q = length(quantile_DSRs),
                            m = length(genome_DSRs),
                            n = length(genome_genes) - length(genome_DSRs),
                            k = length(quantile_genes))
    print(Pval_underrep)
    ...
    if(length(quantile_DSRs) > mean(hgDist)) {
      Pval <- Pval_overrep
    ...
    hgTestResults <- new("hypergeomTest",
                         ...
                         alpha0.05 = alpha0.05,
                         pval = Pval,
                         observed = length(quantile_DSRs),
                         expected = mean(hgDist),
                         log2obsexp = log2( length(quantile_DSRs) / mean(hgDist) ),
                         log2alpha  = log2( alpha0.05 / mean(hgDist) ),
                         quantile_genes = length(quantile_genes),
                         proportion_of_quantile = length(quantile_DSRs) / length(quantile_genes),
                         random_proportions_of_quantile = hgDist / length(quantile_genes),
                         hypergeomDist = hgDist)
    if(libName %in% "cMMb") {
    save(hgTestResults,
         file = paste0(outDir,
                       stressName[xx], "_DSR_gene_representation_among_quantile", z, "_of_", quantileLast,
  ...
  #    textX1 <- min(hgTestResults@hypergeomDist)/1.15
    ...
    mtext(side = 1,
          text = bquote("Genes"),
          line = 1.85)
    titleText <- list(bquote(.(stressName[xx]) ~ "responsive genes in" ~
  ...
  #  scale_y_continuous(limits = c(-1.5, 1.5)) +
    ...
    ggtitle(bquote(.(stressName[xx]) ~ "responsive genes in" ~
  ...
  ggsave(paste0(plotDir,
                "bargraph_", stressName[xx], "_DSR_gene_representation_among_", quantileLast,
...
}, mc.cores = length(stress), mc.preschedule = F)
