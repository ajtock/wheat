...
# with regard to specific bioclimatic variables
# (bioclimatic variable regions; BVR-overlapping genes) in
# each ASY1, DMC1 or cM/Mb  gene quantile are over-represented
# or under-represented
# Bioclimatic variables are as defined at 
# https://worldclim.org/data/bioclim.html and
# as assigned to the regions associated with local adaptation (LARs)
# by He et al. 2019 Nat. Genet. 51
# (i.e., for each of the 19 bioclimatic variables, is the proportion
# of genes in a given quantile that are BVR-overlapping genes
# significantly greater or smaller than expected by chance
# based on the hypergeometric distribution derived from their
# representatiion among BVR-overlapping genes across all quantiles?)
...
# length(genome_BVRs) [m] + ( length(genome_genes) - length(genome_BVRs)) [n] ,
# where quantile_BVRs are BVR-overlapping genes in a given quantile, and
# genome_BVRs are BVR-overlapping genes across all quantiles

# Usage 
# ./proportion_overlapping_bioclim_var_regions_in_gene_quantiles_hypergeometricTest.R 'ASY1_CS_Rep1_ChIP' 'bodies' 1 4 'genomewide' 'Agenome_Bgenome_Dgenome' 100000
...
LARs$env_vars <- gsub(pattern = "_0(\\d\\d)",
                      replacement = "_\\1",
                      x = LARs$env_vars)
bioclimNames <- c(paste0(rep("bio_", 19), 1:19), "alt")
bioclimNames <- gsub(pattern = "_(\\d)$",
                     replacement = "_0\\1",
                     x = bioclimNames)
bioclimNamesPlot <- c(paste0(rep("BIO", 19), 1:19), "Altitude")
...
for(xx in seq_along(bioclimNames)) {
  ...
  # length(quantile_BVRs) to length(quantile_genes)
  ...
  for(z in seq_along(quantile_genes_list)) {
    ...
    set.seed(2847502)
    # Over-representation:
    Pval_overrep <- sum(dhyper(x = length(quantile_BVRs):length(quantile_genes),
                               m = length(genome_BVRs),
                               n = length(genome_genes) - length(genome_BVRs),
    ...
    hgTestResults <- new("hypergeomTest",
                         ...
                         proportion_of_quantile = length(quantile_BVRs) / length(quantile_genes),
                         random_proportions_of_quantile = hgDist / length(quantile_genes),
                         hypergeomDist = hgDist)
    if(libName %in% c("cMMb", "HudsonRM_all")) {
    save(hgTestResults,
         file = paste0(outDir,
                       bioclimNamesPlot[xx], "_LAR2_overlapping_gene_representation_among_quantile", z, "_of_", quantileLast,
  ...
  ggsave(paste0(plotDir,
                "bargraph_", bioclimNamesPlot[xx], "_LAR2_overlapping_gene_representation_among_", quantileLast,
