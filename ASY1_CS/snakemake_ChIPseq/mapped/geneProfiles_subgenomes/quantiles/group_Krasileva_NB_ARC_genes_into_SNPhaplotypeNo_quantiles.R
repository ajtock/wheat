...
# Usage:
# /applications/R/R-3.4.0/bin/Rscript group_Krasileva_NB_ARC_genes_into_SNPhaplotypeNo_quantiles.R 'genes_in_Agenome_genomewide,genes_in_Bgenome_genomewide,genes_in_Dgenome_genomewide' bodies 2 100kb 200
...
outDir <- paste0("quantiles_by_SNPhaplotypeNo_in_",
                 region, "/")
...
# Suppl. data S3
haps <- fread("/home/ajt200/analysis/wheat/annotation/Balfourier_2019_SciAdv_SNP_diversity/Balfourier_et_al_Wheat_Phylogeography_DataS2.tab",
...
haps <- haps[which(!is.na(haps$chromosome)),]
...
hapsGR <- GRanges(seqnames = haps$chromosome,
                  ranges = IRanges(start = haps$snpPosition,
                                   end = haps$snpPosition),
                  strand = "*",
                  hapNo = hapNos)
...
write.table(quantilesStats,
            file = paste0(outDir,
                          "summary_", quantiles, "quantiles_by_SNPhaplotypeNo_in_",
                          region, "_of_",
                          substring(featureName[1][1], first = 1, last = 5), "_in_",
                          paste0(substring(featureName, first = 10, last = 16),
                                 collapse = "_"), "_",
...
#           "_heatmaps_quantiled_by_SNPhaplotypeNo_in_", region, ".pdf"),
