...
for(z in seq_along(quantile_genes_list)) {
  ...
  save(hgTestResults,
       file = paste0(outDir,
                     "meiotic_ortholog_gene_representation_among_quantile", z, "_of_", quantileLast,
...
#    textX1 <- min(hgTestResults@hypergeomDist)/1.15
  ...
  mtext(side = 1,
        text = bquote("Genes"),
        line = 1.85)
  titleText <- list(bquote("Meiotic-ortholog-encoding genes in" ~
...
#  scale_y_continuous(limits = c(-1.5, 1.5)) +
  ...
  ggtitle(bquote("Meiotic-ortholog-encoding genes in" ~
...
ggsave(paste0(plotDir,
              "bargraph_meiotic_ortholog_gene_representation_among_", quantileLast,
