...
featuresDF <- read.table("quantiles_by_H3K27me3_in_genes/WesternEurope/features_4quantiles_by_H3K27me3_in_genes_of_genes_in_Agenome_Bgenome_Dgenome_genomewide_WesternEurope.txt",
...
sapply(seq_along(featureIDsQuantileList), function(k) {
  write.table(featureIDsQuantileList[[k]],
              file = paste0("quantiles_by_H3K27me3_in_genes/",
                            "featureIDs_quantile", k, "_of_", quantiles,
                            "_by_H3K27me3_in_genes_of_",
...
# Each subgenome
sg <-  c("A", "B", "D")
for(x in 1:length(sg)) {
  featureName <- paste0("genes_in_", sg[x], "genome_genomewide")

  quantiles <- 4
  
  featuresDF <- read.table(paste0("quantiles_by_H3K27me3_in_genes/WesternEurope/features_4quantiles_by_H3K27me3_in_genes_of_",
                                  featureName, "_WesternEurope.txt"),
                           header = T, sep = "\t", row.names = NULL, stringsAsFactors = F)
