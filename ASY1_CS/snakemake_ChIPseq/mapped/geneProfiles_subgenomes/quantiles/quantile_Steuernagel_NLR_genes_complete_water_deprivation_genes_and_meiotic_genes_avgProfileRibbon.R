...
subcat1NamePlot <- paste0("NLRs in ",
                          sub("_\\w+", "", dirName),
                          " quantiles")
subcat2NamePlot <- paste0("Drought in ",
                         sub("_\\w+", "", dirName),
                         " quantiles")
...
# Load subcat2
# Load functional annotation in order to extract "cellular response to water deprivation" genes
...
water_deprivation_indices <- which(grepl(pattern = "GO:0042631", x = anno$GO))
water_deprivation_indices <- sort(unique(c(water_deprivation_indices)))
# Retain only "cellular response to water deprivation" genes
# Get "cellular response to water deprivation" gene IDs and their row indices in features
subcat2_IDs <- anno[water_deprivation_indices,]$featureID
