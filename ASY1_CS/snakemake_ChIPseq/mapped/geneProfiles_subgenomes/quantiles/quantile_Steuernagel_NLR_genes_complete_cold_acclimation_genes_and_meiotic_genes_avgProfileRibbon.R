...
subcat1NamePlot <- paste0("NLRs in ",
                          sub("_\\w+", "", dirName),
                          " quantiles")
subcat2NamePlot <- paste0("Cold in ",
                         sub("_\\w+", "", dirName),
                         " quantiles")
...
# Load subcat2
# Load functional annotation in order to extract "cold acclimation" genes
...
cold_acclimation_indices <- which(grepl(pattern = "GO:0009631", x = anno$GO))
cold_acclimation_indices <- sort(unique(c(cold_acclimation_indices)))
# Retain only "cold acclimation" genes
# Get "cold acclimation" gene IDs and their row indices in features
subcat2_IDs <- anno[cold_acclimation_indices,]$featureID
...
if(libName %in% c("cMMb", "HudsonRM_all", "HudsonRM_syn", "HudsonRM_nonsyn")) {
  ggsave(paste0(plotDir,
                ...
                paste0(substring(featureName, first = 10, last = 16),
                       collapse = "_"), "_",
                substring(featureName[1][1], first = 18), "_NLR_Cold_Meiotic_genes_v190620.pdf"),
