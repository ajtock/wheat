...
# Cluster genes by ChIP

# Usage:
# /applications/R/R-3.4.0/bin/Rscript features_heatmap_sorted_kmeans_ChIPonly.R ASY1_CS_Rep1_ChIP ASY1_CS both genes_in_Agenome_genomewide 3500 2000 2kb '2 kb' 20 20bp promoters
...
outDir <- paste0("clusters_by_", libName,
                 "_in_", region, "/")
...
mat1Sorted <- mat1[sort.int(mat1RegionRowMeans,
                            ...
                            index.return = T,
                            na.last = T)$ix,]
# Replace NAs in mat1Region with 0
mat1Region[which(is.na(mat1Region))] <- 0
...
pdf(paste0(outDir, "screePlot_k_clusters_by_",
           libName, "_in_",
...
x <- tapply(X = rowMeans(mat1Region),
            INDEX = km$cluster,
            FUN = mean)
...
#library(clValid)
#km_dunn <- dunn(clusters = km$cluster, Data = mat1Region)

# Order genes in each cluster by decreasing mat1Region levels
# to define "row_order" for heatmaps
combineRowOrders <- function(cluster_bool_list) {
  do.call("c", lapply(cluster_bool_list, function(x) {
    cluster_mat1RegionRowMeans <- rowMeans(mat1Region[x,], na.rm = T)
    which(x)[order(cluster_mat1RegionRowMeans, decreasing = T)]
  }))
}
...
# Order gene IDs in each cluster by decreasing mat1Region levels
...
# Load feature matrices for each chromatin dataset
...
# Convert into normalizedMatrix class
for(x in seq_along(ChIPmats)) {
  attr(ChIPmats[[x]], "upstream_index") = 1:(upstream/binSize)
  attr(ChIPmats[[x]], "target_index") = ((upstream/binSize)+1):((upstream+bodyLength)/binSize)
  attr(ChIPmats[[x]], "downstream_index") = (((upstream+bodyLength)/binSize)+1):(((upstream+bodyLength)/binSize)+(downstream/binSize))
  attr(ChIPmats[[x]], "extend") = c(upstream, downstream)
  attr(ChIPmats[[x]], "smooth") = FALSE
  attr(ChIPmats[[x]], "signal_name") = ChIPNamesPlot[x]
  attr(ChIPmats[[x]], "target_name") = featureName
  attr(ChIPmats[[x]], "target_is_single_point") = FALSE
  attr(ChIPmats[[x]], "background") = 0
  attr(ChIPmats[[x]], "signal_is_categorical") = FALSE
  class(ChIPmats[[x]]) = c("normalizedMatrix", "matrix")
}
...
# Plot together
ChIPhtmpList <- mclapply(seq_along(ChIPNames), function(x) {
  ChIP_col_fun <- colorRamp2(quantile(ChIPmats[[x]],
                                      c(0.5, 0.6, 0.7, 0.8, 0.9, 0.95),
                                      na.rm = T),
                             rich8to6equal)
  featureHeatmap(mat = ChIPmats[[x]],
...
pdf(paste0(plotDir, "ChIP_around_", featureName,
           "_heatmaps_clustered_by_", libName, "_in_", region, ".pdf"),
