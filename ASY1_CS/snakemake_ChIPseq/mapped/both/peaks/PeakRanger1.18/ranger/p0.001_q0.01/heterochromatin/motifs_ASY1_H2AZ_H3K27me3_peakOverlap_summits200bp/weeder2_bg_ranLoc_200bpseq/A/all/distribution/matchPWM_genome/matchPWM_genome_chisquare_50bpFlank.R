...
foreach(x = 1:num_GR) %dopar% {
  ...
  for(h in 1:5) {
    ...
    # Obtain sequences for each motif match, ranLoc and flanking 50-bp regions as DNAStringSet object
    #for(i in 1:10) {
    for(i in 1:length(motif.GRanges[seqnames(motif.GRanges) == h])) {
      matchSeq.chr <- c(matchSeq.chr, DNAStringSet(chr_list[[h]][(start(ranges(motif.GRanges)[seqnames(motif.GRanges) == h][i])-50):(end(ranges(motif.GRanges)[seqnames(motif.GRanges) == h][i])+50)]))
      ranLocSeq.chr <- c(ranLocSeq.chr, DNAStringSet(chr_list[[h]][(start(ranges(ranLoc.GRanges)[seqnames(ranLoc.GRanges) == h][i])-50):(end(ranges(ranLoc.GRanges)[seqnames(ranLoc.GRanges) == h][i])+50)]))
    }
  ...
  write.table(matchSeqPFM, file = paste0(PFMDir, "motif", x, "_matchSeqPFM_50bpFlank.txt"))
  write.table(ranLocSeqPFM, file = paste0(PFMDir, "motif", x, "_ranLocSeqPFM_50bpFlank.txt"))
  ...
  save(chisqList, file = paste0(plotDir, "motif", x, "_chisqList_50bpFlank.RData"))
  ...
  save(chisqDataFrame, file = paste0(plotDir, "motif", x, "_chisqDataFrame_50bpFlank.RData"))
}
