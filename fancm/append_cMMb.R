...
# Usage:
# ./append_cMMb.R cMMb_iwgsc_refseqv1.0_mapping_data_minInterMarkerDist200bp_1Mb_unsmoothed.txt

args <- commandArgs(trailingOnly = T)
inFile <- args[1]

#inFile <- "cMMb_iwgsc_refseqv1.0_mapping_data_minInterMarkerDist200bp_1Mb_unsmoothed.txt"
#inFile <- "iwgsc_refseqv1.0_recombination_rate.txt"
...
cMMb <- read.table(paste0(cMMbDir, inFile), header = T)

colnames(cMMb)[c(1:3, length(colnames(cMMb)))] <- c("chr", "windowStart", "windowEnd", "cMMb")

# Function to convert table into GRanges
...
for(chrName in seqlevels(interGR)) {
  ...
  # cMMb
  chr_cMMb_GR <- cMMb_GR[seqnames(cMMb_GR) == chrName]

#  # Identify overlapping windows
#  fOverlaps_cMMb <- fOverlaps(interGR = chr_interGR, datGR = chr_cMMb_GR)
#
#  # Convert fOverlaps_cMMb* into list object equivalent to that
...
#  fOverlaps_cMMb_list <- mclapply(seq_along(unique(queryHits(fOverlaps_cMMb))),
#                          function(x) {
#                            subjectHits(fOverlaps_cMMb[queryHits(fOverlaps_cMMb) == x])
#                          }, mc.cores = detectCores(), mc.preschedule = T)
  ...
  # for as list elements in subsequently generated fOverlaps_cMMb_list list object
  fOverlaps_cMMb_list <- segmentSeq::getOverlaps(coordinates = chr_interGR,
                                                 segments = chr_cMMb_GR,
                                                 overlapType = "overlapping",
                                                 whichOverlaps = T,
                                                 ignoreStrand = T)
  # Calculate mean methylation proportion in each genomic window (marker interval)
  inter_cMMb <- sapply(fOverlaps_cMMb_list, function(x) {
                  mean(chr_cMMb_GR$val[x], na.rm = T)
                })
