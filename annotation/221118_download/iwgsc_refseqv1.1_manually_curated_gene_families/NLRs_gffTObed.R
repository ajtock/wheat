...
NLRs <- readGFF(paste0(inDir, "nlr.gff3"))
mRNA <- NLRs[NLRs$type == "mRNA",]
print(dim(mRNA))
#[1] 309  12

# Obtain frequency of occurrence of each NLR parent ID
n_occur <- data.frame(table(unlist(mRNA$Parent)))

# Obtain mRNA records for which the NLR parent ID occurs only once
mRNA_unique <-  as.data.frame(
  mRNA[ unlist(mRNA$Parent)
    %in% n_occur$Var1[n_occur$Freq == 1],
  ]
)
# Obtain mRNA records for which the NLR parent ID occurs more than once
...
# For each NLR parent ID in mRNA_multi, obtain the mRNA record with the
# longest transcript
...
write.table(mRNA_rep[,1:9],
            file = paste0(inDir,
                          "IWGSC_v1.1_nlr_representative_mRNA.gff3"),
...
write.table(mRNA_rep_bed,
            file = paste0(inDir,
                          "IWGSC_v1.1_nlr_representative_mRNA.bed"),
            quote = F, sep = "\t", row.names = F, col.names = F)
...
write.table(ranLoc_bed,
            file = paste0(inDir,
                          "IWGSC_v1.1_nlr_representative_mRNA_randomLoci.bed"),
            quote = F, sep = "\t", row.names = F, col.names = F)
